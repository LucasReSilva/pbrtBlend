...
class VolumeExporter(object):
    ...
    def __export_fallback_volume(self):
        # Black clear volume
        self.properties.Set(Property('scene.volumes.' + self.luxcore_name + '.type', 'clear'))
        self.properties.Set(Property('scene.volumes.' + self.luxcore_name + '.absorption', [100, 100, 100]))
        ...
        try:
            ...
            if is_textured:
                ...
                if volume.absorption_scale != 1.0:
                    ...
                    self.properties.Set(Property('scene.textures.' + scale_name + '.type', 'scale'))
                    self.properties.Set(Property('scene.textures.' + scale_name + '.texture1', volume.absorption_scale))
                    self.properties.Set(Property('scene.textures.' + scale_name + '.texture2', abs_col))
                    abs_col = scale_name
            else:
                abs_col = list(absorption_color)
                absorption_at_depth_scaled(abs_col)

            self.properties.Set(Property(prefix + '.absorption', abs_col))
            self.properties.Set(Property(prefix + '.type', volume.type))
            self.properties.Set(Property(prefix + '.ior', ior_val))
            self.properties.Set(Property(prefix + '.priority', volume.priority))
            ...
            if volume.use_emission:
                ...
                if volume.emission_usecolortexture:
                    if volume.gain != 1.0:
                        ...
                        self.properties.Set(Property('scene.textures.' + scale_name + '.type', 'scale'))
                        self.properties.Set(Property('scene.textures.' + scale_name + '.texture1', emission_color))
                        self.properties.Set(Property('scene.textures.' + scale_name + '.texture2', volume.gain))
                ...
                self.properties.Set(Property(prefix + '.emission', emission_color))
            ...
            if volume.type in ['homogeneous', 'heterogeneous']:
                ...
                if volume.sigma_s_usecolortexture:
                    ...
                    if volume.scattering_scale != 1.0:
                        self.properties.Set(Property('scene.textures.' + self.luxcore_name + '_scatterscaling.type', 'scale'))
                        self.properties.Set(Property('scene.textures.' + self.luxcore_name + '_scatterscaling.texture1',
                                                             volume.scattering_scale))
                        self.properties.Set(Property('scene.textures.' + self.luxcore_name + '_scatterscaling.texture2', scattering_col))
                ...
                self.properties.Set(Property(prefix + '.scattering', s_col))
                self.properties.Set(Property(prefix + '.asymmetry', list(volume.g)))
                self.properties.Set(Property(prefix + '.multiscattering', [volume.multiscattering]))
    
            if volume.type == 'heterogeneous':
                self.properties.Set(Property(prefix + '.steps.size', volume.stepsize))
        except Exception as err:
            ...
            self.properties.Set(Property(prefix + '.type', 'clear'))