...
class lightgroups_base(object):
    ...
    def draw(self, context):
        if UsePBRTv3Core() and not hasattr(pypbrtv3core.RenderSession, 'Parse'):
        ...
        def draw_lightgroup(lg, lg_index=None):
            ...
            if not self.is_imageeditor_panel and not is_default_group:
                ...
                row.operator('pbrtv3render.lightgroup_remove', text="", icon="X", emboss=False).lg_index = lg_index
        ...
        if not self.is_imageeditor_panel:
            self.layout.operator('pbrtv3render.lightgroup_add', text='Add Lightgroup', icon='ZOOMIN')
...
class materialgroups(render_layers_panel):
    ...
    def draw(self, context):
        ...
        def draw_materialgroup(mg, mg_index=None):
            ...
            row.operator('pbrtv3render.materialgroup_remove', text='', icon='X', emboss=False).mg_index = mg_index
        ...
        self.layout.operator('pbrtv3render.materialgroup_add', text='Add Materialgroup', icon='ZOOMIN')
