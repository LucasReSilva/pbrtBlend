...
def node_tree_selector_draw(layout, id_data, output_type):
    ...
    if not node:
        if not id_data.pbrtv3_material.nodetree:
            ...
            sub_layout.operator('pbrtv3render.add_material_nodetree', icon='NODETREE')
            prop_search_text = ''
    ...
    def draw(self, context):
        ...
        if ob:
            ...
            if slot:
                ...
                split.operator("pbrtv3render.material_copy")
...
@PBRTv3Addon.addon_register_class
class ui_pbrtv3core_material(pbrtv3_material_base):
    """
    ...
    display_property_groups = [
        ( ('material',), 'pbrtv3core_material', lambda: UsePBRTv3Core() ),
    ]
...
class ui_pbrtv3_material_utils(pbrtv3_material_base):
    ...
    def draw(self, context):
        row = self.layout.row(align=True)
        row.operator("pbrtv3render.load_material", icon="DISK_DRIVE")
        row.operator("pbrtv3render.save_material", icon="DISK_DRIVE").filename = \
        ...
        sub.operator("pbrtv3render.convert_cycles_material", icon='MATERIAL_DATA')
        column.operator("pbrtv3render.convert_all_cycles_materials", icon='WORLD_DATA')

        column = self.layout.column(align=True)
        column.operator("pbrtv3render.convert_material", icon='MATERIAL_DATA')
        column.operator("pbrtv3render.convert_all_materials", icon='WORLD_DATA')

    # row.operator("pbrtv3render.material_reset", icon='SOLID')
...
#             self.layout.operator('pbrtv3render.lrmdb', text='Enable').invoke_action_id = -1
#         else:
#             self.layout.operator('pbrtv3render.lrmdb', text='Disable').invoke_action_id = -2
#
#             for action in lrmdb_state.actions:
#                 if action.callback is None:
#                     self.layout.label(text=action.label)
#                 else:
#                     self.layout.operator('pbrtv3render.lrmdb', text=action.label).invoke_action_id = action.aid
#
#     @classmethod
#     def poll(cls, context):
#         if context.scene.render.engine != 'PBRTv3_RENDER':
