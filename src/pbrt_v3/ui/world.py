...
class volumes_base(object):
    ...
    def draw(self, context):
        ...
        # row.operator("pbrtv3render.preset_volume_add", text="", icon="ZOOMIN")
        # row.operator("pbrtv3render.preset_volume_add", text="", icon="ZOOMOUT").remove_active = True
        ...
        if len(context.scene.pbrtv3_volumes.volumes) > 0:
            ...
            if current_vol.nodetree:
                ...
                col.operator('pbrtv3render.add_volume_nodetree', icon='NODETREE')
                if not UsePBRTv3Core():
