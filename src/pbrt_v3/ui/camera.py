...
class camera(camera_panel):
    ...
    def draw(self, context):
        ...
        pbrtv3_cam = context.camera.pbrtv3_camera

        # Draw property groups
        super().draw(context)

        if pbrtv3_cam.use_dof and not pbrtv3_cam.usemblur:
            # mblur already has a trailing separator if enabled
            layout.separator()

        layout.prop(pbrtv3_cam, "use_dof", toggle=True)

        if pbrtv3_cam.use_dof:
            split = layout.split()

            column = split.column()
            column.label("Focus:")
            column.prop(pbrtv3_cam, "autofocus")

            # Disable "Distance" and "Object" settings if autofocus is used
            sub_autofocus = column.column()
            sub_autofocus.enabled = not pbrtv3_cam.autofocus
            ...
            if UsePBRTv3Core():
                ...
                sub_bokeh.prop(pbrtv3_cam, "blades", text="Blades")
                sub_bokeh.prop(pbrtv3_cam, "distribution", text="")
                sub_bokeh.prop(pbrtv3_cam, "power", text="Power")

            layout.label("DoF strength is controlled by f/Stop value", icon="INFO")

        if UsePBRTv3Core():
            if pbrtv3_cam.enable_clipping_plane or pbrtv3_cam.use_dof:
                layout.separator()

            layout.prop(pbrtv3_cam, "enable_clipping_plane", toggle=True)

            if pbrtv3_cam.enable_clipping_plane:
                layout.prop_search(pbrtv3_cam, "clipping_plane_obj", context.scene, "objects", text="Plane")
...
class film(camera_panel):
    ...
    display_property_groups = [
        ...
        ( ('camera', 'pbrtv3_camera'), 'pbrtv3core_imagepipeline', lambda: UsePBRTv3Core() ),
    ]

    def draw_crf_preset_menu(self, context):
        if UsePBRTv3Core():
            self.layout.menu('IMAGEPIPELINE_MT_pbrtv3_crf',
                         text=context.camera.pbrtv3_camera.pbrtv3core_imagepipeline.crf_preset)
        else:
    ...
    def draw(self, context):
        ...
        if UsePBRTv3Core():
            imagepipeline_settings = context.scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline
