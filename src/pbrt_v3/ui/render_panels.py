...
from ..outputs.pbrtv3core_api import UsePBRTv3Core, pypbrtv3core
...
class render_settings(render_panel):
    ...
    display_property_groups = [
        ...
        ( ('scene',), 'pbrtv3core_enginesettings', lambda: UsePBRTv3Core() ),
    ]
    ...
    def draw(self, context):
        ...
        if not UsePBRTv3Core():
            ...
            row.operator("pbrtv3render.preset_engine_add", text="", icon="ZOOMIN")
            row.operator("pbrtv3render.preset_engine_add", text="", icon="ZOOMOUT").remove_active = True
        ...
        if UsePBRTv3Core():
            ...
            if vs.view_transform != 'Default' or vs.exposure != 0 or vs.gamma != 1 or vs.look != 'None' or vs.use_curve_mapping:
                ...
                layout.operator('pbrtv3render.fix_color_management')
...
class device_settings(render_panel):
    ...
    def draw(self, context):
        ...
        if (render_mode in ['hybridpath', 'pbrtv3corepathocl', 'pbrtv3corebiaspathocl'] and not UsePBRTv3Core()) \
            ...
            if UsePBRTv3Core() and engine_settings.opencl_settings_type == 'SIMPLE':
                ...
                if UsePBRTv3Core():
                    self.layout.prop(context.scene.pbrtv3core_enginesettings, 'use_opencl_always_enabled')
                    self.layout.prop(context.scene.pbrtv3core_enginesettings, 'film_use_opencl')
                    self.layout.prop(context.scene.pbrtv3core_enginesettings, 'kernelcache')

                self.layout.operator('pbrtv3render.opencl_device_list_update')

                # This is a "special" panel section for the list of OpenCL devices
                if len(context.scene.pbrtv3core_enginesettings.pbrtv3core_opencl_devices) > 0:
                    for dev in context.scene.pbrtv3core_enginesettings.pbrtv3core_opencl_devices:
                        row = self.layout.row()
        ...
        # Tile settings
        if UsePBRTv3Core() and context.scene.pbrtv3core_enginesettings.renderengine_type == 'TILEPATH':
...
class translator(render_panel):
    ...
    display_property_groups = [
        ( ('scene',), 'pbrtv3core_scenesettings', lambda: UsePBRTv3Core() ),
        ( ('scene',), 'pbrtv3_engine', lambda: not UsePBRTv3Core() ),
        ( ('scene',), 'pbrtv3_testing', lambda: not UsePBRTv3Core() ),
        ( ('scene',), 'pbrtv3core_translatorsettings', lambda: UsePBRTv3Core() ),
    ]
    ...
    def draw(self, context):
        if not UsePBRTv3Core():
            ...
            self.layout.prop(context.scene.pbrtv3core_enginesettings, 'custom_properties')
...
class networking(render_panel):
    ...
    def draw(self, context):
        if not UsePBRTv3Core():
            ...
            row.operator("pbrtv3render.preset_networking_add", text="", icon="ZOOMIN")
            row.operator("pbrtv3render.preset_networking_add", text="", icon="ZOOMOUT").remove_active = True
        else:
