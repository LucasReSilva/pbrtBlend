...
class imageeditor_panel(property_group_renderer):
    ...
    def poll(cls, context):
        engine_is_pbrtv3 = context.scene.render.engine in cls.COMPAT_ENGINES
        return engine_is_pbrtv3 and UsePBRTv3Core()
...
class rendering_controls_panel(imageeditor_panel):
    ...
    def draw(self, context):
        if context.scene.pbrtv3core_rendering_controls.pause_render:
        ...
        self.layout.prop(context.scene.pbrtv3core_rendering_controls, 'pause_render', toggle=True, text=button_text,
                         icon=button_icon)
...
class tonemapping_panel(imageeditor_panel):
    ...
    def draw(self, context):
        ...
        if not hasattr(pypbrtv3core.RenderSession, 'Parse'):
        ...
        pbrtv3_cam = context.scene.camera.data.pbrtv3_camera
        imagepipeline_settings = pbrtv3_cam.pbrtv3core_imagepipeline
        ...
        if imagepipeline_settings.tonemapper_type == 'TONEMAP_LINEAR':
            ...
            layout.prop(pbrtv3_cam, 'sensitivity')
...
class halt_conditions_panel(imageeditor_panel):
    ...
    def poll(cls, context):
        ...
        return engine_is_pbrtv3 and UsePBRTv3Core() and context.scene.pbrtv3core_enginesettings.renderengine_type != 'TILEPATH'

    def draw(self, context):
        layout = self.layout
        settings = context.scene.pbrtv3core_enginesettings
...
class rendering_statistics_panel(imageeditor_panel):
    ...
    def draw(self, context):
        box = self.layout.box()
        for elem in context.scene.pbrtv3core_rendering_controls.controls:
            box.prop(context.scene.pbrtv3core_rendering_controls, elem)

        if bpy.context.scene.pbrtv3core_enginesettings.renderengine_type == 'TILEPATH':
            box = self.layout.box()
            box.prop(context.scene.pbrtv3core_tile_highlighting, 'use_tile_highlighting')

            if context.scene.pbrtv3core_tile_highlighting.use_tile_highlighting:
                box.separator()
                box.prop(context.scene.pbrtv3core_tile_highlighting, 'show_converged')
                box.prop(context.scene.pbrtv3core_tile_highlighting, 'show_unconverged')
                box.prop(context.scene.pbrtv3core_tile_highlighting, 'show_pending')