...
class pbrtv3_ui_rendering_controls(property_group_renderer):
    ...
    def poll(cls, context):
        ...
        if eng:
            ...
            if lma:
                cls.ctx = LM.GetActive().pbrtv3_context
    ...
    def draw(self, context):
        ...
        if self.ctx is not None and self.ctx.API_TYPE == 'PURE':
            pypbrtv3 = self.ctx.PYLUX
            tm_data = context.scene.camera.data.pbrtv3_camera.pbrtv3_film.pbrtv3_tonemapping
            tm_map = {
                'reinhard': pypbrtv3.ImageFilm.TonemapKernels.Reinhard,
                'linear': pypbrtv3.ImageFilm.TonemapKernels.Linear,
                'autolinear': pypbrtv3.ImageFilm.TonemapKernels.AutoLinear,
                'contrast': pypbrtv3.ImageFilm.TonemapKernels.Contrast,
                'maxwhite': pypbrtv3.ImageFilm.TonemapKernels.MaxWhite,
            }
