...
from ..outputs.pbrtv3core_api import UsePBRTv3Core
...
def pbrtv3_bumpmap_export(self, pbrtv3_context, material, bumpmap_material_name, TF_bumpmap, TF_normalmap):
    ...
    if self.normalmap_usefloattexture:  # We have a normal map
        ...
        if texture_name and self.normalmap_usefloattexture:
            ...
            pbrtv3_texture = texture.pbrtv3_texture
            params = ParamSet()

            if pbrtv3_texture.type in ('normalmap', 'imagemap'):
                if pbrtv3_texture.type == 'normalmap':
                    src_texture = pbrtv3_texture.pbrtv3_tex_normalmap
                else:
                    src_texture = pbrtv3_texture.pbrtv3_tex_imagemap
                ...
                params.update(pbrtv3_texture.pbrtv3_tex_mapping.get_paramset(PBRTv3Manager.CurrentScene))
            elif pbrtv3_texture.type == 'BLENDER' and texture.type == 'IMAGE':
    ...
    if self.normalmap_usefloattexture and self.bumpmap_usefloattexture:
        ...
        if texture_name:
            ...
            if pbrtv3_texture.type == 'BLENDER' and texture.type == 'IMAGE':
        ...
        if PBRTv3Manager.GetActive().pbrtv3_context.API_TYPE == 'PURE':
...
class pbrtv3_material(declarative_property_group):
    ...
    def exportNodetree(self, scene, pbrtv3_context, material, mode):
        ...
        return outputNode.export(scene, pbrtv3_context, material, mode)
    ...
    def export(self, scene, pbrtv3_context, material, mode='indirect'):
        ...
        if self.nodetree:
            return self.exportNodetree(scene, pbrtv3_context, material, mode)
        ...
        with MaterialCounter(material.name):
            if not (mode == 'indirect' and material.name in ExportedMaterials.exported_material_names):
                if self.type == 'mix':
                    ...
                    m1.pbrtv3_material.export(scene, pbrtv3_context, m1, 'indirect')
                    ...
                    m2.pbrtv3_material.export(scene, pbrtv3_context, m2, 'indirect')
                ...
                if self.type == 'glossycoating':
                    ...
                    bm.pbrtv3_material.export(scene, pbrtv3_context, bm, 'indirect')
                ...
                if self.type == 'layered':
                    ...
                    if m3_name:
                        ...
                        m3.pbrtv3_material.export(scene, pbrtv3_context, m3, 'indirect')

                    m4_name = self.pbrtv3_mat_layered.namedmaterial4_material

                    if m4_name:
                        m4 = bpy.data.materials[m4_name]
                        m4.pbrtv3_material.export(scene, pbrtv3_context, m4, 'indirect')
                ...
                if hasattr(material, 'pbrtv3_transparency') and material.pbrtv3_transparency.transparent:
                    alpha_type, alpha_amount = material.pbrtv3_transparency.export(pbrtv3_context, material)

                coating_params = None

                # find coating if material should be coated
                if hasattr(material, 'pbrtv3_coating') and material.pbrtv3_coating.use_coating:
                    coating_params = material.pbrtv3_coating.export(pbrtv3_context, material)

                # Bump and normal mapping
                if self.type not in ['mix', 'null', 'layered']:
                    material_params.update(
                        pbrtv3_bumpmap_export(self, pbrtv3_context, material, material.name, TF_bumpmap, TF_normalmap))

                if hasattr(subtype, 'export'):
                    subtype.export(pbrtv3_context, material)
                ...
                if coating_params is not None:
                    ...
                    ExportedMaterials.makeNamedMaterial(pbrtv3_context, material.name + '_base', material_params)
                ...
                if alpha_type is not None:
                    ...
                    material_params.add_string('type', mat_type)
                    ExportedMaterials.makeNamedMaterial(pbrtv3_context, material.name + '_null',
                                                        ParamSet().add_string('type', 'null'))
                    ExportedMaterials.makeNamedMaterial(pbrtv3_context, material.name + '_mat', material_params)
                    ExportedMaterials.export_new_named(pbrtv3_context)
                ...
                if mode == 'indirect':
                    material_params.add_string('type', mat_type)
                    ExportedMaterials.makeNamedMaterial(pbrtv3_context, material.name, material_params)
                    ExportedMaterials.export_new_named(pbrtv3_context)
                elif mode == 'direct':
                    pbrtv3_context.material(mat_type, material_params)
...
class pbrtv3_transparency(declarative_property_group):
    ...
    def export(self, pbrtv3_context, material):
        pbrtv3_material = getattr(material.pbrtv3_material, 'pbrtv3_mat_%s' % material.pbrtv3_material.type)
        alpha_type = None
        ...
        if self.alpha_source == 'texture':
            ...
            texture_name = getattr(pbrtv3_material,
                                   '%s_colortexturename' % self.sourceMap[material.pbrtv3_material.type])
            if texture_name:
                texture = get_texture_from_scene(PBRTv3Manager.CurrentScene, texture_name)
                pbrtv3_texture = texture.pbrtv3_texture

                if pbrtv3_texture.type == 'imagemap':
                    src_texture = pbrtv3_texture.pbrtv3_tex_imagemap
...
class pbrtv3_coating(declarative_property_group):
    ...
    def export(self, pbrtv3_context, material):
        ...
        glossycoating_params.update(
            pbrtv3_bumpmap_export(self, pbrtv3_context, material, material.name + '_coating', TF_c_bumpmap,
                                     TF_c_normalmap))
        ...
        if self.metaltype == 'fresnelcolor':
            ...
            if PBRTv3Manager.GetActive() is not None:
                fresnelcolor_params.update(
                    add_texture_parameter(PBRTv3Manager.GetActive().pbrtv3_context, 'Kr', 'color', self)
                )
