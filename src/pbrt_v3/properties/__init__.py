...
import bpy
from ..outputs.pbrtv3core_api import ToValidPBRTv3CoreName, UsePBRTv3Core, set_prop_mat, set_prop_vol
...
def warning_pbrtv3core_node(layout):
    """
...
def create_pbrtv3core_name(node, suffix=None, name=None):
    """
...
def create_pbrtv3core_name_mat(node, name=None):
    return create_pbrtv3core_name(node, 'mat', name)

def create_pbrtv3core_name_vol(node, name=None):
    return create_pbrtv3core_name(node, 'vol', name)

def export_fallback_material(properties, socket, name):
    # Black matte material
    pbrtv3core_name = create_pbrtv3core_name_mat(socket, name)
...
def export_fallback_volume(properties, socket, name):
    # Black clear volume
    pbrtv3core_name = create_pbrtv3core_name_vol(socket, name)
    set_prop_vol(properties, pbrtv3core_name, 'type', 'clear')
    set_prop_vol(properties, pbrtv3core_name, 'absorption', [100, 100, 100])
    return pbrtv3core_name

def export_submat_pbrtv3core(properties, socket, pbrtv3core_exporter, name=None):
    """
    ...
    if node is None:
        ...
        submat_name = node.export_pbrtv3core(properties, pbrtv3core_exporter, name)

    return submat_name

def export_volume_pbrtv3core(properties, socket, name=None):
    """
    NodeSocketShader sockets cannot export themselves, so this function does it
    """
    node = get_linked_node(socket)
    ...
    if node is None:
        ...
        volume_name = node.export_pbrtv3core(properties, name)

    return volume_name

def export_emission_pbrtv3core(properties, pbrtv3core_exporter, socket, parent_pbrtv3core_name, is_volume_emission=False):
    """
    NodeSocketShader sockets cannot export themselves, so this function does it
    """
    node = get_linked_node(socket)
    ...
    if node is not None:
        node.export_pbrtv3core(properties, pbrtv3core_exporter, parent_pbrtv3core_name, is_volume_emission)