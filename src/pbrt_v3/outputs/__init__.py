...
def PBRTv3Log(*args, popup=False):
    ...
    Accepts variable args (can be used as pypbrtv3.errorHandler)
    """
    if len(args) > 0:
        log(' '.join(['%s' % a for a in args]), module_name='Lux', popup=popup)

# CHOOSE API TYPE
# Write conventional lx* files and use pypbrtv3 to manage pbrtv3 process or external process
...
# Access pbrtv3 only through pypbrtv3 bindings
from ..outputs import pure_api


class LuxAPIStats(TimerThread):
    """
    Periodically get pbrtv3 stats
    """
    ...
    def kick(self):
        ctx = self.LocalStorage['pbrtv3_context']
    ...
    def kick(self, render_end=False):
        if 'RE' in self.LocalStorage.keys():
            ...
            if not bpy.app.background or render_end:
                ...
                if 'pbrtv3_context' in self.LocalStorage.keys() and self.LocalStorage['pbrtv3_context'].statistics(
                        'sceneIsReady') > 0.0:
                    self.LocalStorage['pbrtv3_context'].updateFramebuffer()
                    xres = int(self.LocalStorage['pbrtv3_context'].getAttribute('film', 'xResolution'))
                    yres = int(self.LocalStorage['pbrtv3_context'].getAttribute('film', 'yResolution'))
                    ctx = self.LocalStorage['pbrtv3_context']
                    ctx.updateStatisticsWindow()
                    p_stats = ' - %s' % ctx.getAttribute('renderer_statistics_formatted_short', '_recommended_string')
                    direct_transfer = 'blenderCombinedDepthRects' in dir(self.LocalStorage['pbrtv3_context'])
                ...
                if direct_transfer:
                    # use the framebuffer direct from pypbrtv3 using a special method
...
class PBRTv3Manager(object):
    """
    Manage a pypbrtv3.Context object for rendering.

    Objects of this class are responsible for the life cycle of
    a pypbrtv3.Context object, ensuring proper initialisation, usage
    and termination.
    ...
    def __init__(self, manager_name='', api_type='FILE'):
        """
        Initialise the PBRTv3Manager by setting its name, the pypbrtv3 API type.

        Returns PBRTv3Manager object
        """

        # NB: Should not be refactored into a dict switch as it will
        # lead to undefined context in case of pypbrtv3 module import fail.
        if api_type == 'FILE':
        ...
        self.pbrtv3_context = Context('LuxContext %04i%s' %
    ...
    def start(self):
        """
        Start the pypbrtv3.Context object rendering. This is achieved
        ...
        while self.pbrtv3_context.statistics('sceneIsReady') != 1.0:
    ...
    def reset(self):
        ...
        if self.pbrtv3_context is not None:
            self.pbrtv3_context.exit()
            self.pbrtv3_context.wait()
        ...
        self.stats_thread = LuxAPIStats(
            {'pbrtv3_context': self.pbrtv3_context}
        )
        ...
        if self.pbrtv3_context is not None:
            ...
            for server_info in self.pbrtv3_context.getRenderingServersStatus():
                self.pbrtv3_context.removeServer(server_info.name)

            # cleanup() destroys the pypbrtv3 Context
            self.pbrtv3_context.cleanup()
    ...
    def __del__(self):
        """
        Gracefully exit() pbrtv3 upon destruction
        """
        self.reset()
