...
from ..outputs.pbrtv3core_api import PYpbrtv3core_AVAILABLE, UsePBRTv3Core, pypbrtv3core
from ..export.pbrtv3core import PBRTv3CoreExporter
from ..export.pbrtv3core.utils import get_elem_key
...
from ..properties import (
    ...
    pbrtv3core_engine, pbrtv3core_scene, pbrtv3core_material, pbrtv3core_lamp,
    pbrtv3core_tile_highlighting, pbrtv3core_imagepipeline, pbrtv3core_translator, pbrtv3core_rendering_controls, pbrtv3core_global
)
...
from ..ui.textures import (
    ...
    lampspectrum, pbrtv3pop, marble, mix as tex_mix, multimix, sellmeier, scale, subtract, sopra, uv,
...
def pbrtv3_output_hints(self, context):
...
_register_elm(bl_ui.properties_render.RENDER_PT_output.append(pbrtv3_output_hints))


# Add view buttons for viewcontrol to preview panels
def pbrtv3_use_alternate_matview(self, context):
...
_register_elm(bl_ui.properties_material.MATERIAL_PT_preview.append(pbrtv3_use_alternate_matview))


def pbrtv3_use_alternate_texview(self, context):
    if context.scene.render.engine == 'PBRTv3_RENDER':
        row = self.layout.row()
...
_register_elm(bl_ui.properties_texture.TEXTURE_PT_preview.append(pbrtv3_use_alternate_texview))


# Add use_clipping button to lens panel
def pbrtv3_use_clipping(self, context):
    if context.scene.render.engine == 'PBRTv3_RENDER':
...
_register_elm(bl_ui.properties_data_camera.DATA_PT_lens.append(pbrtv3_use_clipping))
...
def render_start_options(self, context):
    if context.scene.render.engine == 'PBRTv3_RENDER':
        ...
        if UsePBRTv3Core():
            col.prop(context.scene.pbrtv3core_translatorsettings, "export_type")
            if context.scene.pbrtv3core_translatorsettings.export_type == "pbrtv3coreui":
                sub = col.row()
                sub.alignment = 'RIGHT'
                sub.prop(context.scene.pbrtv3core_translatorsettings, "run_pbrtv3coreui")
        else:
        ...
        split.operator("pbrtv3render.convert_cycles_scene", icon='EXPORT')
        # Usability design is crap, but it has to do for now
        split.prop(context.scene.pbrtv3core_global, 'cycles_converter_fallback_color', text='')
...
def pbrtv3_texture_chooser(self, context):
    if context.scene.render.engine == 'PBRTv3_RENDER':
...
_register_elm(bl_ui.properties_texture.TEXTURE_PT_context_texture.append(pbrtv3_texture_chooser))
...
def DrawButtonPause(self, context):
    ...
    if scene.render.engine == "PBRTv3_RENDER" and UsePBRTv3Core():
        ...
        if view.viewport_shade == "RENDERED":
            self.layout.prop(scene.pbrtv3core_rendering_controls, "pause_viewport_render", icon="PAUSE", text="")
...
def draw_button_show_imagemap_previews(self, context):
    if context.scene.render.engine == "PBRTv3_RENDER" and UsePBRTv3Core():
        self.layout.prop(context.scene.pbrtv3core_global, 'nodeeditor_show_imagemap_previews',
                         toggle=True,
                         icon='IMAGE_COL')
...
class RENDERENGINE_pbrtv3render(bpy.types.RenderEngine):
    """
    ...
    def render(self, scene):
        ...
        with RENDERENGINE_pbrtv3render.render_lock:  # just render one thing at a time
            if scene is None:
                PBRTv3Log('ERROR: Scene to render is not valid')
                return

            if UsePBRTv3Core():
                self.pbrtv3core_render(scene)
            else:
                self.pbrtv3_render(scene)

    def view_update(self, context):
        if UsePBRTv3Core():
            self.pbrtv3core_view_update(context)
        else:
            self.update_stats('ERROR: Viewport rendering is only available when PBRTv3Core API is selected!', '')

    def view_draw(self, context):
        if UsePBRTv3Core():
            self.pbrtv3core_view_draw(context)
    ...
    def pbrtv3_render(self, scene):
        ...
        try:
            ...
            make_queue = scene.pbrtv3_engine.export_type == 'EXT' and \
                         scene.pbrtv3_engine.binary_name == 'pbrtv3render' and write_files
            ...
            if is_animation and make_queue:
                ...
                self.PBRTv3Manager.pbrtv3_context.worldEnd()
    ...
    def pbrtv3_render_preview(self, scene):
        ...
        if not PYLUX_AVAILABLE:
            PBRTv3Log('ERROR: Material previews require pypbrtv3')
            return
        ...
        if file_based_preview:
            ...
            preview_context.set_filename(scene, 'pbrtv3blend25-preview', LXV=False)
            LM.pbrtv3_context = preview_context
        else:
            preview_context = LM.pbrtv3_context
        ...
        try:
            ...
            if file_based_preview:
                preview_context = preview_context.parse('pbrtv3blend25-preview.lxs', True)
                LM.pbrtv3_context = preview_context
        ...
        # cleanup() destroys the pypbrtv3 Context
    ...
    def export_scene(self, scene):
        ...
        if scene.pbrtv3_engine.export_type == 'INT':
            ...
            if scene.pbrtv3_networking.use_network_servers and scene.pbrtv3_networking.servers:
                LM.pbrtv3_context.setNetworkServerUpdateInterval(scene.pbrtv3_networking.serverinterval)
                for server in scene.pbrtv3_networking.servers.split(','):
                    LM.pbrtv3_context.addServer(server.strip())
    ...
    def append_pbrtv3_binary_name(self, scene, pbrtv3_path, binary_name):
        if sys.platform == 'darwin':
    ...
    def get_pbrtv3_binary_path(self, scene, binary_name):
        pbrtv3_path = efutil.filesystem_path(efutil.find_config_value(
                              'pbrtv3render', 'defaults', 'install_path', ''))
        ...
        pbrtv3_path = self.append_pbrtv3_binary_name(scene, pbrtv3_path, binary_name)
    ...
    def get_process_args(self, scene, start_rendering):
        ...
        pbrtv3_path = self.get_pbrtv3_binary_path(scene, scene.pbrtv3_engine.binary_name)
        ...
        if scene.pbrtv3_engine.binary_name != 'pbrtv3vr':
        ...
        if scene.pbrtv3_engine.binary_name == 'pbrtv3render':
        ...
        try:
            for k, v in config_updates.items():
                efutil.write_config_value('pbrtv3render', 'defaults', k, v)
        except Exception as err:
    ...
    def render_start(self, scene):
        ...
        if self.PBRTv3Manager.pbrtv3_context.API_TYPE == 'FILE':
            fn = self.PBRTv3Manager.pbrtv3_context.file_names[0]
            self.PBRTv3Manager.pbrtv3_context.worldEnd()
            if parse:
                # file_api.parse() creates a real pypbrtv3 context. we must replace
                # PBRTv3Manager's context with that one so that the running renderer
                # can be controlled.
                ctx = self.PBRTv3Manager.pbrtv3_context.parse(fn, True)
                self.PBRTv3Manager.pbrtv3_context = ctx
                self.PBRTv3Manager.stats_thread.LocalStorage['pbrtv3_context'] = ctx
                self.PBRTv3Manager.fb_thread.LocalStorage['pbrtv3_context'] = ctx
        elif worldEnd:
            self.PBRTv3Manager.pbrtv3_context.worldEnd()
        ...
        if start_rendering:
            ...
            if internal:
                ...
                self.PBRTv3Manager.pbrtv3_context.logVerbosity(scene.pbrtv3_engine.log_verbosity)
                ...
                for i in range(thread_count - 1):
                    self.PBRTv3Manager.pbrtv3_context.addThread()
                ...
                if not (
                        scene.pbrtv3_engine.binary_name == 'pbrtv3render' and not
                    ...
                    # If we exit the wait loop (user cancelled) and pbrtv3console is still running, then send SIGINT
                    if pbrtv3_process.poll() is None and scene.pbrtv3_engine.binary_name != 'pbrtv3render':
    ...
    def stats_timer(self):
        ...
        LC = self.PBRTv3Manager.pbrtv3_context
    ...
    def set_export_path_pbrtv3core(self, scene):
        # replace /tmp/ with the real %temp% folder on Windows
        # OSX also has a special temp location that we should use
        fp = scene.render.filepath
        output_path_split = list(os.path.split(fp))
    ...
    def CreateBlenderStats(self, lcConfig, stats, scene, realtime_preview=False, time_until_update=-1, display_interval=-1,
        ...
        """
        rendering_controls = scene.pbrtv3core_rendering_controls
        ...
        settings = scene.pbrtv3core_enginesettings
        ...
        if scene.pbrtv3core_rendering_controls.pause_render:
    ...
    def haltConditionMet(self, scene, stats, realtime_preview=False):
        ...
        return halt_samples_met or halt_time_met or halt_noise_met or scene.pbrtv3core_rendering_controls.pause_viewport_render
    ...
    def convertChannelToImage(self, lcSession, scene, passes, filmWidth, filmHeight, channelType, saveToDisk,
        ...
        # Structure: {channelType: [pypbrtv3coreType, is HDR, arrayDepth, optional matching Blender pass]}
        attributes = {
                'RGB': [pypbrtv3core.FilmOutputType.RGB, True, 3],
                'RGBA': [pypbrtv3core.FilmOutputType.RGBA, True, 4],
                'RGB_TONEMAPPED': [pypbrtv3core.FilmOutputType.RGB_TONEMAPPED, False, 3],
                'RGBA_TONEMAPPED': [pypbrtv3core.FilmOutputType.RGBA_TONEMAPPED, False, 4],
                'ALPHA': [pypbrtv3core.FilmOutputType.ALPHA, False, 1],
                'DEPTH': [pypbrtv3core.FilmOutputType.DEPTH, True, 1, 'Z'],
                'POSITION': [pypbrtv3core.FilmOutputType.POSITION, True, 3],
                'GEOMETRY_NORMAL': [pypbrtv3core.FilmOutputType.GEOMETRY_NORMAL, True, 3],
                'SHADING_NORMAL': [pypbrtv3core.FilmOutputType.SHADING_NORMAL, True, 3, 'NORMAL'],
                'MATERIAL_ID': [pypbrtv3core.FilmOutputType.MATERIAL_ID, False, 1],
                'OBJECT_ID': [pypbrtv3core.FilmOutputType.OBJECT_ID, False, 1],
                'DIRECT_DIFFUSE': [pypbrtv3core.FilmOutputType.DIRECT_DIFFUSE, True, 3, 'DIFFUSE_DIRECT'],
                'DIRECT_GLOSSY': [pypbrtv3core.FilmOutputType.DIRECT_GLOSSY, True, 3, 'GLOSSY_DIRECT'],
                'EMISSION': [pypbrtv3core.FilmOutputType.EMISSION, True, 3, 'EMIT'],
                'INDIRECT_DIFFUSE': [pypbrtv3core.FilmOutputType.INDIRECT_DIFFUSE, True, 3, 'DIFFUSE_INDIRECT'],
                'INDIRECT_GLOSSY': [pypbrtv3core.FilmOutputType.INDIRECT_GLOSSY, True, 3, 'GLOSSY_INDIRECT'],
                'INDIRECT_SPECULAR': [pypbrtv3core.FilmOutputType.INDIRECT_SPECULAR, True, 3, 'TRANSMISSION_INDIRECT'],
                'DIRECT_SHADOW_MASK': [pypbrtv3core.FilmOutputType.DIRECT_SHADOW_MASK, False, 1],
                'INDIRECT_SHADOW_MASK': [pypbrtv3core.FilmOutputType.INDIRECT_SHADOW_MASK, False, 1],
                'UV': [pypbrtv3core.FilmOutputType.UV, True, 2],
                'RAYCOUNT': [pypbrtv3core.FilmOutputType.RAYCOUNT, True, 1],
                'IRRADIANCE': [pypbrtv3core.FilmOutputType.IRRADIANCE, True, 3],
                'MATERIAL_ID_MASK': [pypbrtv3core.FilmOutputType.MATERIAL_ID_MASK, False, 1],
                'BY_MATERIAL_ID': [pypbrtv3core.FilmOutputType.BY_MATERIAL_ID, True, 3],
                'RADIANCE_GROUP': [pypbrtv3core.FilmOutputType.RADIANCE_GROUP, True, 3]
        }
        ...
        if channelType in ('MATERIAL_ID', 'OBJECT_ID'):
            ...
            if pass_type is not None and scene.pbrtv3_channels.import_compatible:
                ...
                if arrayDepth == 1:
                    channel_buffer_converted = pypbrtv3core.ConvertFilmChannelOutput_1xFloat_To_4xFloatList(filmWidth,
                                                                                                         filmHeight,
                                                                                                         channel_buffer,
                                                                                                         normalize)
                # UV channel, just add 0.0 for B and 1.0 for A components
                elif arrayDepth == 2:
                    channel_buffer_converted = pypbrtv3core.ConvertFilmChannelOutput_2xFloat_To_4xFloatList(filmWidth,
                                                                                                         filmHeight,
                                                                                                         channel_buffer,
                                                                                                         normalize)
                # RGB channels: just add 1.0 as alpha component
                elif arrayDepth == 3:
                    channel_buffer_converted = pypbrtv3core.ConvertFilmChannelOutput_3xFloat_To_4xFloatList(filmWidth,
                                                                                                         filmHeight,
                                                                                                         channel_buffer,
                                                                                                         normalize)
    ...
    def draw_tiles(self, scene, stats, imageBuffer, filmWidth, filmHeight):
        ...
        """
        tile_size = scene.pbrtv3core_enginesettings.tile_size
        show_converged = scene.pbrtv3core_tile_highlighting.show_converged
        show_unconverged = scene.pbrtv3core_tile_highlighting.show_unconverged
        show_pending = scene.pbrtv3core_tile_highlighting.show_pending
    ...
    def pbrtv3core_render(self, scene):
        if self.is_preview:
            self.pbrtv3core_render_preview(scene)
            return

        # PBRTv3Core libs
        if not PYpbrtv3core_AVAILABLE:
            PBRTv3Log('ERROR: PBRTv3Core rendering requires pypbrtv3core')
            self.report({'ERROR'}, 'PBRTv3Core rendering requires pypbrtv3core')
            return
        from ..outputs.pbrtv3core_api import pypbrtv3core

        try:
            scene.pbrtv3core_rendering_controls.pause_render = False
            ...
            self.set_export_path_pbrtv3core(scene)
            # Stay compatible to old pypbrtv3core versions
            self.transparent_film = (scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline.transparent_film and
                                     hasattr(pypbrtv3core, 'ConvertFilmChannelOutput_4xFloat_To_4xFloatList'))

            filmWidth, filmHeight = self.get_film_size(scene)

            pbrtv3core_exporter = PBRTv3CoreExporter(scene, self)
            pbrtv3core_config = pbrtv3core_exporter.convert(filmWidth, filmHeight)

            # Maybe export was cancelled by user, don't start the rendering with an incomplete scene then
            if self.test_break() or pbrtv3core_config is None:
                return

            pbrtv3core_session = pypbrtv3core.RenderSession(pbrtv3core_config)
            # Start the rendering
            PBRTv3Log('Starting the rendering process...')
            pbrtv3core_session.Start()

            # Print a summary of errors that happened during export, if there are any
            pbrtv3core_exporter.error_cache.print_errors()
            export_errors = pbrtv3core_exporter.error_cache.contains_errors()

            # Immediately end the rendering if 'FILESAVER' engine is used
            if scene.pbrtv3core_translatorsettings.export_type == 'pbrtv3coreui':
                pbrtv3core_session.Stop()

                if scene.pbrtv3core_translatorsettings.run_pbrtv3coreui:
                    pbrtv3_path = self.get_pbrtv3_binary_path(scene, 'pbrtv3coreui')
            ...
            imagepipeline_settings = scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline
            ...
            session_props = pbrtv3core_exporter.convert_imagepipeline()
            session_props.Set(pbrtv3core_exporter.convert_lightgroup_scales())
            ...
            while not self.test_break() and not done:
                ...
                new_session_props = pbrtv3core_exporter.convert_imagepipeline()
                new_session_props.Set(pbrtv3core_exporter.convert_lightgroup_scales())

                if str(new_session_props) != cached_session_props:
                    cached_session_props = str(new_session_props)

                    # Safety check for old pypbrtv3core versions compatibility
                    pbrtv3core_session.Parse(new_session_props)
                    print('Set session settings:\n%s' % cached_session_props)
                    session_was_updated = True

                # Pause/Resume of rendering prcess
                if scene.pbrtv3core_rendering_controls.pause_render:
                    if not pbrtv3core_session.IsInPause():
                        pbrtv3core_session.UpdateStats()

                        pbrtv3core_session.Pause()
                        print('Rendering paused.')

                        stats = pbrtv3core_session.GetStats()
                        result = self.create_result(pbrtv3core_session, imageBufferFloat, scene, stats, filmWidth, filmHeight, False)
                        self.end_result(result)
                        last_image_display = now
                else:
                    if pbrtv3core_session.IsInPause():
                        pbrtv3core_session.Resume()
                ...
                blender_stats = self.CreateBlenderStats(pbrtv3core_config, stats, scene, False, time_until_update,
            ...
            stats = pbrtv3core_session.GetStats()
            result = self.create_result(pbrtv3core_session, imageBufferFloat, scene, stats, filmWidth, filmHeight, True)
            ...
            if scene.pbrtv3_channels.enable_aovs:
                if scene.pbrtv3_channels.saveToDisk:
                    ...
                    pbrtv3core_session.GetFilm().Save()

                if scene.pbrtv3_channels.import_into_blender:
                    self.import_aov_channels(scene, pbrtv3core_session, filmWidth, filmHeight, result.layers[0].passes)
    ...
    def create_result(self, pbrtv3core_session, imageBufferFloat, scene, stats, filmWidth, filmHeight, is_final_result):
        """
        ...
        if self.transparent_film:
            output_type = pypbrtv3core.FilmOutputType.RGBA_TONEMAPPED
            def convert(filmWidth, filmHeight, imageBufferFloat):
                ...
                return pypbrtv3core.ConvertFilmChannelOutput_4xFloat_To_4xFloatList(filmWidth, filmHeight, imageBufferFloat, False)
        else:
            output_type = pypbrtv3core.FilmOutputType.RGB_TONEMAPPED
            def convert(filmWidth, filmHeight, imageBufferFloat):
                return pypbrtv3core.ConvertFilmChannelOutput_3xFloat_To_3xFloatList(filmWidth, filmHeight, imageBufferFloat)

        # Update the image
        pbrtv3core_session.GetFilm().GetOutputFloat(output_type, imageBufferFloat)
        ...
        if (scene.pbrtv3core_enginesettings.renderengine_type == 'TILEPATH' and
                scene.pbrtv3core_tile_highlighting.use_tile_highlighting and not is_final_result):
    ...
    def pbrtv3core_render_preview(self, scene):
        # PBRTv3Core libs
        if not PYpbrtv3core_AVAILABLE:
            PBRTv3Log('ERROR: PBRTv3Core preview rendering requires pypbrtv3core')
            return
        from ..outputs.pbrtv3core_api import pypbrtv3core
        from ..export.pbrtv3core.materialpreview import MaterialPreviewExporter

        try:
            def update_result(pbrtv3core_session, imageBufferFloat, filmWidth, filmHeight):
                # Update the image
                pbrtv3core_session.GetFilm().GetOutputFloat(pypbrtv3core.FilmOutputType.RGB_TONEMAPPED, imageBufferFloat)

                # Here we write the pixel values to the RenderResult
                result = self.begin_result(0, 0, filmWidth, filmHeight)
                layer = result.layers[0] if bpy.app.version < (2, 74, 4) else result.layers[0].passes[0]

                layer.rect = pypbrtv3core.ConvertFilmChannelOutput_3xFloat_To_3xFloatList(filmWidth, filmHeight,
                                                                                   imageBufferFloat)
                self.end_result(result)
            ...
            pbrtv3core_config = exporter.convert(filmWidth, filmHeight)

            if pbrtv3core_config is None:
                return
            ...
            if is_thumbnail or preview_type == 'TEXTURE':
                ...
                while not self.test_break() and time.time() - startTime < stopTime and not done:
                    time.sleep(0.1)
                    update_result(pbrtv3core_session, imageBufferFloat, filmWidth, filmHeight)
    ...
    def pbrtv3core_view_draw(self, context):
        ...
        if context.scene.camera:
            interval = context.scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline.viewport_interval / 1000
        else:
            interval = 0.05
        ...
        # PBRTv3Core libs
        if not PYpbrtv3core_AVAILABLE:
            PBRTv3Log('ERROR: PBRTv3Core real-time rendering requires pypbrtv3core')
            return
        ...
        if (self.viewFilmWidth != context.region.width) or (
            ...
            self.pbrtv3core_view_update(context, update_changes)

        # check if camera settings have changed
        self.pbrtv3core_exporter.convert_camera()
        newCameraSettings = str(self.pbrtv3core_exporter.pop_updated_scene_properties())
        ...
        if PBRTv3CoreSessionManager.is_session_active(self.space):
            session.pbrtv3core_session.UpdateStats()
            stats = session.pbrtv3core_session.GetStats()
            ...
            if not session.is_in_scene_edit():
                ...
                pbrtv3core_config = session.pbrtv3core_session.GetRenderConfig()
                blender_stats = self.CreateBlenderStats(pbrtv3core_config, stats, context.scene, realtime_preview=True)
        ...
        session.pbrtv3core_session.WaitNewFrame()
        session.pbrtv3core_session.GetFilm().GetOutputFloat(output_type, self.viewImageBufferFloat)
        draw_framebuffer()
    ...
    def find_update_changes(self, context):
        ...
        try:
                ...
                for mat in bpy.data.materials:
                    ...
                    if nodetree_name and nodetree_name in bpy.data.node_groups:
                        ...
                        if nodetree.is_updated or nodetree.is_updated_data:
                            self.pbrtv3core_exporter.convert_material(mat)
                            newNodeMatSettings = str(self.pbrtv3core_exporter.pop_updated_scene_properties())
            ...
            for volume in context.scene.pbrtv3_volumes.volumes:
                self.pbrtv3core_exporter.convert_volume(volume)
            ...
            newVolumeProperties = self.pbrtv3core_exporter.pop_updated_scene_properties()
            ...
            newHaltTime = context.scene.pbrtv3core_enginesettings.halt_time_preview
            newHaltSamples = context.scene.pbrtv3core_enginesettings.halt_samples_preview
            ...
            self.pbrtv3core_exporter.convert_config(self.viewFilmWidth, self.viewFilmHeight)
            newRenderSettings = str(self.pbrtv3core_exporter.config_exporter.properties)
            ...
            session_props = self.pbrtv3core_exporter.convert_imagepipeline()
            session_props.Set(self.pbrtv3core_exporter.convert_lightgroup_scales())
    ...
    def pbrtv3core_view_update(self, context, update_changes=None):
        # PBRTv3Core libs
        if not PYpbrtv3core_AVAILABLE:
            PBRTv3Log('ERROR: PBRTv3Core real-time rendering requires pypbrtv3core')
            return

        if self.test_break() or context.scene.pbrtv3core_rendering_controls.pause_viewport_render:
            return
        ...
        if update_changes.cause_unknown:
            ...
            try:
                ...
                PBRTv3CoreSessionManager.stop_pbrtv3core_session(self.space)

                if context.scene.camera:
                    self.transparent_film = context.scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline.transparent_film
                else:
                ...
                pbrtv3core_config = self.pbrtv3core_exporter.convert(self.viewFilmWidth, self.viewFilmHeight)
                if pbrtv3core_config is None:
                    PBRTv3Log('ERROR: not a valid pbrtv3core config')
                    return

                PBRTv3CoreSessionManager.create_pbrtv3core_session(pbrtv3core_config, self.space)
                PBRTv3CoreSessionManager.start_pbrtv3core_session(self.space)

                self.critical_errors = False
            except Exception as exc:
                # This flag is used to prevent the pbrtv3core_draw() function from crashing Blender
            ...
            if update_changes.cause_config:
                ...
                pbrtv3core_config = PBRTv3CoreSessionManager.get_session(self.space).pbrtv3core_session.GetRenderConfig()
                PBRTv3CoreSessionManager.stop_pbrtv3core_session(self.space)

                self.pbrtv3core_exporter.convert_config(self.viewFilmWidth, self.viewFilmHeight)

                # change config
                pbrtv3core_config.Parse(self.pbrtv3core_exporter.config_properties)
                if pbrtv3core_config is None:
                    PBRTv3Log('ERROR: not a valid pbrtv3core config')
                    return
            ...
            if update_changes.scene_edit_necessary:
                # begin sceneEdit
                pbrtv3core_scene = PBRTv3CoreSessionManager.get_session(self.space).pbrtv3core_session.GetRenderConfig().GetScene()
                ...
                if update_changes.cause_materials:
                    ...
                    for material in update_changes.changed_materials:
                        self.pbrtv3core_exporter.convert_material(material)

                if update_changes.cause_mesh:
                    for ob in update_changes.changed_objects_mesh:
                        PBRTv3Log('Mesh update: ' + ob.name)
                        self.pbrtv3core_exporter.convert_object(ob, pbrtv3core_scene, update_mesh=True, update_material=False)
                ...
                if update_changes.cause_light or update_changes.cause_objectTransform:
                    for ob in update_changes.changed_objects_transform:
                        ...
                        self.pbrtv3core_exporter.convert_object(ob, pbrtv3core_scene, update_mesh=False, update_material=False)
                ...
                if update_changes.cause_objectsRemoved:
                    for ob in update_changes.removed_objects:
                        ...
                        if ob.type == 'LAMP':
                            if key in self.pbrtv3core_exporter.light_cache:
                                # In case of sunsky there might be multiple light sources, loop through them
                                for exported_light in self.pbrtv3core_exporter.light_cache[key].exported_lights:
                                    pbrtv3core_name = exported_light.pbrtv3core_name

                                    if exported_light.type == 'AREA':
                                        # Area lights are meshlights and treated like objects with glowing materials
                                        pbrtv3core_scene.DeleteObject(pbrtv3core_name)
                                    else:
                                        pbrtv3core_scene.DeleteLight(pbrtv3core_name)
                        else:
                            if key in self.pbrtv3core_exporter.object_cache:
                                # loop through object components (split by materials)
                                for exported_object in self.pbrtv3core_exporter.object_cache[key].exported_objects:
                                    pbrtv3core_name = exported_object.pbrtv3core_object_name
                                    pbrtv3core_scene.DeleteObject(pbrtv3core_name)
                ...
                updated_properties = self.pbrtv3core_exporter.pop_updated_scene_properties()

                if context.space_data.local_view:
                    # Add a uniform white background light in local view so we have a lightsource
                    updated_properties.Set(pypbrtv3core.Property('scene.lights.LOCALVIEW_BACKGROUND.type', 'constantinfinite'))
                else:
                    pbrtv3core_scene.DeleteLight('LOCALVIEW_BACKGROUND')

                if context.scene.pbrtv3core_translatorsettings.print_scn:
                    # Debug output
                    print('Updated scene properties:')
                    print(updated_properties, '\n')

                # parse scene changes and end sceneEdit
                pbrtv3core_scene.Parse(updated_properties)
            ...
            if update_changes.cause_session:
                ...
                props = self.pbrtv3core_exporter.convert_imagepipeline()
                props.Set(self.pbrtv3core_exporter.convert_lightgroup_scales())

                PBRTv3CoreSessionManager.get_session(self.space).pbrtv3core_session.Parse(props)
...
class Session(object):
    ...
    """
    def __init__(self, pbrtv3core_session):
        self.pbrtv3core_session = pbrtv3core_session
        self.is_active = False

    def is_in_scene_edit(self):
        return self.pbrtv3core_session.IsInSceneEdit()

    def is_paused(self):
        return self.pbrtv3core_session.IsInPause()
...
class PBRTv3CoreSessionManager(object):
    ...
    @classmethod
    def create_pbrtv3core_session(cls, pbrtv3core_config, space):
        # space: bpy.context.screen.areas[x].spaces[y] (areas[x].type == "VIEW_3D", spaces[y].type == "VIEW_3D")
        cls.sessions[space] = Session(pypbrtv3core.RenderSession(pbrtv3core_config))
    ...
    @classmethod
    def start_pbrtv3core_session(cls, space):
        if space in cls.sessions:
            session = cls.sessions[space]

            if not session.is_active:
                print('Starting viewport render')
                session.pbrtv3core_session.Start()
                session.is_active = True

    @classmethod
    def stop_pbrtv3core_session(cls, space):
        if space in cls.sessions:
            session = cls.sessions[space]
            ...
            if session.is_active:
                ...
                session.pbrtv3core_session.Stop()
                session.pbrtv3core_session = None
    ...
    def stop_orphaned_sessions(cls):
        ...
        for space, session in orphans:
            ...
            if session.is_in_scene_edit():
                session.pbrtv3core_session.EndSceneEdit()
    ...
    def begin_scene_edit(cls, space):
        if space in cls.sessions:
            ...
            if session.is_active and not session.is_in_scene_edit():
                ...
                session.pbrtv3core_session.BeginSceneEdit()
    ...
    def pause(cls, space):
        if space in cls.sessions:
            ...
            if session.is_active and not session.is_paused():
                ...
                session.pbrtv3core_session.Pause()
    ...
    def resume(cls, space):
        if space in cls.sessions:
            ...
            if session.is_active and session.is_paused():
                ...
                session.pbrtv3core_session.Resume()
...
def stop_viewport_render(context):
    ...
    for space in spaces:
        if space is not None and space.viewport_shade != 'RENDERED':
            PBRTv3CoreSessionManager.stop_pbrtv3core_session(space)
