...
bl_info = {
    ...
    "wiki_url": "http://www.pbrtv3render.net/wiki/LuxBlend25_Manual",
    "tracker_url": "http://www.pbrtv3render.net/mantis",
    "description": "PBRTv3 integration for Blender"
}

def find_pbrtv3_path():
    path = efutil.find_config_value('pbrtv3render', 'defaults', 'install_path', '')
...
def import_bindings_module(name):
    """Import Lux Python bindings module (e.g. pypbrtv3)."""
    import os.path
    import sys
    import importlib
    ...
    pbrtv3_path = find_pbrtv3_path()
    pbrtv3blend_path = os.path.dirname(os.path.abspath(__file__))
    if sys.platform == 'darwin':
        return _import_bindings_module(pbrtv3blend_path, name, True)
    else:
        try:
            module = _import_bindings_module(pbrtv3_path, name)
        except ImportError as error:
            PBRTv3Log('Failed to import {} module from "{}" (Error: {})'.format(name, pbrtv3_path, error))
            module = _import_bindings_module(pbrtv3blend_path, name, True)
        return module
...
if 'core' in locals():
    ...
    def set_pbrtv3_path(self, path):
        ...
        if not path:
            return
        efutil.write_config_value('pbrtv3render', 'defaults', 'install_path',
                                  efutil.filesystem_path(path))

    def get_pbrtv3_path(self):
        """Load Lux install path from persistent disk storage."""
        return efutil.find_config_value('pbrtv3render', 'defaults',
                                        'install_path', '')
    ...
    class PBRTv3AddonPreferences(AddonPreferences):
        ...
        def draw(self, context):
            ...
            split.label(text="After editing these settings please restart Blender for the changes to take effect.",
                        icon='INFO')
            split.operator("pbrtv3render.update_pbrtv3blend", icon='RECOVER_AUTO')

            split = layout.split(percentage=0.78)
            split.template_reports_banner()
            split.label(text="Updating LuxBlend only updates the addon, not the PBRTv3 binaries. Use this instead:")
            split.operator("pbrtv3render.open_daily_builds_webpage", icon='URL')
