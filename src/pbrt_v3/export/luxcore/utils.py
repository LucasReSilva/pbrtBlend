...
def convert_texture_channel(pbrtv3core_exporter, properties, element_name, textured_element, channel, type):
    """
    :param pbrtv3core_exporter: the pbrtv3core_exporter instance of the calling texture/volume/material exporter
    :param properties: pypbrtv3core.Properties
    :param element_name: name of the pbrtv3render material/volume/texture (PBRTv3Core name)
    :param textured_element: pbrtv3render material, volume, texture or anything else with attributes that can be textured
    ...
    if getattr(textured_element, '%s_use%stexture' % (channel, type)):
        ...
        texture = get_texture_from_scene(pbrtv3core_exporter.blender_scene, texture_name)

        if texture:
            pbrtv3core_exporter.convert_texture(texture)
            texture_exporter = pbrtv3core_exporter.texture_cache[get_elem_key(texture)]

            is_multiplied = getattr(textured_element, '%s_multiply%s' % (channel, type))

            if is_multiplied:
                scale_tex_name = element_name + channel + type + texture_exporter.pbrtv3core_name
                if channel == 'bumpmap':
                    # we must multiply bump with worldscale
                    ws = get_worldscale(as_scalematrix=False)
                    value =  [x * ws for x in value]

                create_scale_texture(properties, texture_exporter.pbrtv3core_name, scale_tex_name, value)
                return scale_tex_name
            else:
                return texture_exporter.pbrtv3core_name
    else:
...
def create_scale_texture(properties, base_texture_name, scale_texture_name, multiplier):
    properties.Set(pypbrtv3core.Property('scene.textures.' + scale_texture_name + '.type', 'scale'))
    properties.Set(pypbrtv3core.Property('scene.textures.' + scale_texture_name + '.texture1', base_texture_name))
    properties.Set(pypbrtv3core.Property('scene.textures.' + scale_texture_name + '.texture2', multiplier))


def convert_param_to_pbrtv3core_property(param):
    """
...
def calc_shutter(blender_scene, pbrtv3_camera_settings):
    fps = blender_scene.render.fps / blender_scene.render.fps_base

    if pbrtv3_camera_settings.exposure_mode == 'normalised':
        shutter_open = pbrtv3_camera_settings.exposure_start_norm / fps
        shutter_close = pbrtv3_camera_settings.exposure_end_norm / fps
    elif pbrtv3_camera_settings.exposure_mode == 'absolute':
        shutter_open = pbrtv3_camera_settings.exposure_start_abs
        shutter_close = pbrtv3_camera_settings.exposure_end_abs
    elif pbrtv3_camera_settings.exposure_mode == 'degrees':
        shutter_open = pbrtv3_camera_settings.exposure_degrees_start / (fps * 2 * pi)
        shutter_close = pbrtv3_camera_settings.exposure_degrees_end / (fps * 2 * pi)
    else:
        raise Exception('exposure mode "%s" not supported' % pbrtv3_camera_settings.exposure_mode)
...
def is_lightgroup_opencl_compatible(pbrtv3core_exporter, lightgroup_id):
    """
    ...
    """
    engine_settings = pbrtv3core_exporter.blender_scene.pbrtv3core_enginesettings
    is_opencl_engine = not engine_settings.renderengine_type.startswith('BIDIR') and (
                        (engine_settings.device == 'OCL' and not pbrtv3core_exporter.is_viewport_render) or (
                        engine_settings.device_preview == 'OCL' and pbrtv3core_exporter.is_viewport_render))
...
def log_exception(pbrtv3core_exporter, message):
    print(message)
    import traceback
    traceback.print_exc()
    pbrtv3core_exporter.error_cache.add_error(message, traceback.format_exc())
