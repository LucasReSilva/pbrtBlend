...
class ExportedObject(object):
    def __init__(self, name, shape_name, material_name):
        self.pbrtv3core_object_name = name
        self.pbrtv3core_shape_name = shape_name
        self.pbrtv3core_material_name = material_name


class ObjectExporter(object):
    def __init__(self, pbrtv3core_exporter, blender_scene, is_viewport_render=False, blender_object=None,
                 dupli_name_suffix=''):
    ...
    def convert(self, update_mesh, update_material, pbrtv3core_scene, anim_matrices=None, matrix=None):
        self.properties = pypbrtv3core.Properties()

        self.__convert_object(pbrtv3core_scene, update_mesh, update_material, anim_matrices, matrix)
    ...
    def __use_instancing(self, anim_matrices):
        ...
        if (not ((obj.parent and obj.parent.is_duplicator) or
                         obj in self.pbrtv3core_exporter.instanced_duplis)) and \
                         obj.data.users == 1:
            return False
    ...
    def __convert_object(self, pbrtv3core_scene, update_mesh, update_material, anim_matrices, matrix):
        obj = self.blender_object
        ...
        if obj.type == 'LAMP':
            self.pbrtv3core_exporter.convert_light(self.blender_object, pbrtv3core_scene)
            return
        ...
        export_proxies = self.blender_scene.pbrtv3core_translatorsettings.export_proxies
        ...
        if (len(obj.particle_systems) > 0 or obj.is_duplicator) and not self.is_dupli:
            if get_elem_key(obj) not in self.pbrtv3core_exporter.dupli_cache:
                if obj.parent and obj.parent.is_duplicator:
                    # Go all the way up to the highest duplicator
                    if get_elem_key(obj.parent) not in self.pbrtv3core_exporter.dupli_cache:
                        self.pbrtv3core_exporter.convert_object(obj.parent, pbrtv3core_scene, update_mesh, update_material)
                        return
                else:
                    self.pbrtv3core_exporter.convert_duplis(pbrtv3core_scene, obj)
        ...
        if MeshExporter.get_mesh_key(obj, self.is_viewport_render, use_instancing) in self.pbrtv3core_exporter.mesh_cache:
            # Check if object is in cache
            if get_elem_key(obj) in self.pbrtv3core_exporter.object_cache and update_mesh and not self.is_dupli:
                self.pbrtv3core_exporter.convert_mesh(obj, pbrtv3core_scene, use_instancing, transform)
    ...
    def __update_props(self, anim_matrices, obj, transform, update_material):
        mesh_exporter = self.pbrtv3core_exporter.mesh_cache[MeshExporter.get_mesh_key(obj, self.is_viewport_render,
                                                                                   self.__use_instancing(anim_matrices))]
        self.__create_pbrtv3core_objects(mesh_exporter.exported_shapes, transform, update_material, anim_matrices)
    ...
    def __convert_proxy(self, update_material, anim_matrices, convert_object, transform):
        ...
        # Convert material
        if update_material or get_elem_key(self.blender_object.active_material) not in self.pbrtv3core_exporter.material_cache:
            self.pbrtv3core_exporter.convert_material(self.blender_object.active_material)

        material_exporter = self.pbrtv3core_exporter.material_cache[get_elem_key(self.blender_object.active_material)]
        pbrtv3core_material_name = material_exporter.pbrtv3core_name

        # Create shape definition (always instanciate proxies)
        name_shape = ToValidPBRTv3CoreName(os.path.basename(path) + '_pbrtv3blendproxy')
        self.properties.Set(pypbrtv3core.Property('scene.shapes.' + name_shape + '.type', 'mesh'))
        self.properties.Set(pypbrtv3core.Property('scene.shapes.' + name_shape + '.ply', path))
        self.__create_object_properties(name, name_shape, pbrtv3core_material_name, transform, anim_matrices)


    def __create_pbrtv3core_objects(self, exported_shapes, transform, update_material, anim_matrices):
        self.exported_objects = []
        ...
        for shape in exported_shapes:
            ...
            # Convert material
            if update_material or get_elem_key(material) not in self.pbrtv3core_exporter.material_cache:
                self.pbrtv3core_exporter.convert_material(material)
            material_exporter = self.pbrtv3core_exporter.material_cache[get_elem_key(material)]
            pbrtv3core_material_name = material_exporter.pbrtv3core_name

            self.__create_object_properties(name, shape.pbrtv3core_shape_name, pbrtv3core_material_name, transform, anim_matrices)


    def __handle_pointiness(self, pbrtv3core_shape_name):
        ...
        if use_pointiness:
            pointiness_shape = pbrtv3core_shape_name + '_pointiness'
            self.properties.Set(pypbrtv3core.Property('scene.shapes.' + pointiness_shape + '.type', 'pointiness'))
            self.properties.Set(pypbrtv3core.Property('scene.shapes.' + pointiness_shape + '.source', pbrtv3core_shape_name))
            pbrtv3core_shape_name = pointiness_shape

        return pbrtv3core_shape_name


    def __create_object_properties(self, pbrtv3core_object_name, pbrtv3core_shape_name, pbrtv3core_material_name, transform, anim_matrices):
        # Insert a pointiness shape if a pointiness texture is used in one of the materials/textures
        pbrtv3core_shape_name = self.__handle_pointiness(pbrtv3core_shape_name)

        self.exported_objects.append(ExportedObject(pbrtv3core_object_name, pbrtv3core_shape_name, pbrtv3core_material_name))

        prefix = 'scene.objects.' + pbrtv3core_object_name

        self.properties.Set(pypbrtv3core.Property(prefix + '.material', pbrtv3core_material_name))
        self.properties.Set(pypbrtv3core.Property(prefix + '.shape', pbrtv3core_shape_name))
        ...
        if transform is not None and self.__use_instancing(anim_matrices) and not use_motion_blur:
            ...
            self.properties.Set(pypbrtv3core.Property(prefix + '.transformation', transform))
        ...
        if use_motion_blur:
            ...
            for i in range(len(anim_matrices)):
                ...
                self.properties.Set(pypbrtv3core.Property('%s.motion.%d.time' % (prefix, i), time))
                self.properties.Set(pypbrtv3core.Property('%s.motion.%d.transformation' % (prefix, i), matrix))
    ...
    def __calc_motion_blur(self):
        ...
        if pbrtv3_camera.usemblur and pbrtv3_camera.objectmblur:
            steps = pbrtv3_camera.motion_blur_samples
