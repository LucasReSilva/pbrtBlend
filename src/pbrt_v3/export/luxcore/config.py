...
class ConfigExporter(object):
    def __init__(self, pbrtv3core_exporter, blender_scene, is_viewport_render=False):
    ...
    def convert_channel(self, channelName, id=-1, lightgroup_name=''):
        ...
        """
        if channelName in self.pbrtv3core_exporter.passes_cache:
            return
        else:
            self.pbrtv3core_exporter.passes_cache.add(channelName)
        ...
        if channelName == 'RADIANCE_GROUP':
            if not is_lightgroup_opencl_compatible(self.pbrtv3core_exporter, id):
                # don't create the output channel
        ...
        self.properties.Set(pypbrtv3core.Property(outputStringType, [channelName]))
        ...
        self.properties.Set(pypbrtv3core.Property(outputStringFilename, path))

        # output id
        if id != -1:
            outputStringId = 'film.outputs.' + str(self.outputCounter) + '.id'
            self.properties.Set(pypbrtv3core.Property(outputStringId, [id]))
    ...
    def get_engine(self):
        ...
        """
        engine_settings = self.blender_scene.pbrtv3core_enginesettings
    ...
    def __convert_film_size(self, film_width, film_height):
        self.properties.Set(pypbrtv3core.Property('film.width', film_width))
        self.properties.Set(pypbrtv3core.Property('film.height', film_height))
    ...
    def __convert_seed(self):
        ...
        self.properties.Set(pypbrtv3core.Property('renderengine.seed', seed))
    ...
    def __convert_halt_conditions(self):
        ...
        if engine_settings.use_halt_noise:
            ...
            self.properties.Set(pypbrtv3core.Property('batch.haltthreshold', haltthreshold))
            # All other halt conditions are controlled in core/__init__.py and not set via pbrtv3core properties
    ...
    def __convert_sampler(self):
        ...
        if self.get_engine() == 'RTPATHCPU':
            ...
            self.properties.Set(pypbrtv3core.Property('sampler.type', 'RTPATHCPUSAMPLER'))
        elif self.get_engine() in ('TILEPATHCPU', 'TILEPATHOCL', 'RTPATHOCL'):
            # (RT)TILEPATHOCL needs a special sampler
            self.properties.Set(pypbrtv3core.Property('sampler.type', 'TILEPATHSAMPLER'))
        else:
            self.properties.Set(pypbrtv3core.Property('sampler.type', [engine_settings.sampler_type]))

        if engine_settings.advanced and engine_settings.sampler_type == 'METROPOLIS':
            self.properties.Set(pypbrtv3core.Property('sampler.metropolis.largesteprate', [engine_settings.largesteprate]))
            self.properties.Set(
                pypbrtv3core.Property('sampler.metropolis.maxconsecutivereject', [engine_settings.maxconsecutivereject]))
            self.properties.Set(
                pypbrtv3core.Property('sampler.metropolis.imagemutationrate', [engine_settings.imagemutationrate]))
    
    
    def __convert_filter(self):
        engine_settings = self.blender_scene.pbrtv3core_enginesettings
    
        self.properties.Set(pypbrtv3core.Property('film.filter.type', [engine_settings.filter_type]))
        self.properties.Set(pypbrtv3core.Property('film.filter.width', [engine_settings.filter_width]))
    ...
    def __convert_accelerator(self):
        ...
        engine_settings = self.blender_scene.pbrtv3core_enginesettings
        self.properties.Set(pypbrtv3core.Property('accelerator.instances.enable', engine_settings.instancing))
    ...
    def __convert_engine(self):
        ...
        if self.blender_scene.camera:
            export_to_pbrtv3coreui = self.blender_scene.pbrtv3core_translatorsettings.export_type == 'pbrtv3coreui'
            transparent_film = self.blender_scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline.transparent_film
            force_black_background = transparent_film and not export_to_pbrtv3coreui
        else:
            force_black_background = False

        if self.blender_scene.pbrtv3core_translatorsettings.export_type == 'pbrtv3coreui' and not self.is_viewport_render:
            # efutil.export_path is set at the beginning of the render() function in core/__init__.py
            self.properties.Set(pypbrtv3core.Property('renderengine.type', 'FILESAVER'))
            self.properties.Set(pypbrtv3core.Property('filesaver.directory', efutil.export_path))
            self.properties.Set(pypbrtv3core.Property('filesaver.renderengine.type', engine))
        else:
            self.properties.Set(pypbrtv3core.Property('renderengine.type', engine))
        ...
        if engine in ['TILEPATHCPU', 'TILEPATHOCL']:
            self.properties.Set(pypbrtv3core.Property('path.clamping.variance.maxvalue', radiance_clamp))
            self.properties.Set(pypbrtv3core.Property('path.clamping.pdf.value', pdf_clamp))

            self.properties.Set(pypbrtv3core.Property('tile.size', [engine_settings.tile_size]))
            self.properties.Set(pypbrtv3core.Property('tile.multipass.enable',
                                                 [engine_settings.tile_multipass_enable]))
            self.properties.Set(pypbrtv3core.Property('tile.multipass.convergencetest.threshold',
            ...
            self.properties.Set(pypbrtv3core.Property('tilepath.forceblackbackground.enable', force_black_background))

            self.properties.Set(pypbrtv3core.Property('tile.multipass.convergencetest.threshold.reduction',
            ...
            self.properties.Set(pypbrtv3core.Property('tilepath.sampling.aa.size', aa_samples))

            if engine == 'TILEPATHCPU':
                self.properties.Set(pypbrtv3core.Property('tilepath.sampling.diffuse.size', diffuse_samples))
                self.properties.Set(pypbrtv3core.Property('tilepath.sampling.glossy.size', glossy_samples))
                self.properties.Set(pypbrtv3core.Property('tilepath.sampling.specular.size', specular_samples))

            # Path depths, note that for non-specular paths +1 is added to the path depth.
            # For details see http://www.pbrtv3render.net/forum/viewtopic.php?f=11&t=11101&start=390#p114959
            self.properties.Set(pypbrtv3core.Property('path.pathdepth.total',
                                                 [engine_settings.path_pathdepth_total + 1]))
            self.properties.Set(pypbrtv3core.Property('path.pathdepth.diffuse',
                                                 [engine_settings.path_pathdepth_diffuse + 1]))
            self.properties.Set(pypbrtv3core.Property('path.pathdepth.glossy',
                                                 [engine_settings.path_pathdepth_glossy + 1]))
            self.properties.Set(pypbrtv3core.Property('path.pathdepth.specular',
                                                 [engine_settings.path_pathdepth_specular]))
            self.properties.Set(pypbrtv3core.Property('path.lights.samplingstrategy.type',
            ...
            self.properties.Set(pypbrtv3core.Property('path.clamping.pdf.value', pdf_clamp))
            # Transparent film enables using black bg
            self.properties.Set(pypbrtv3core.Property('path.forceblackbackground.enable', force_black_background))
        elif engine in ['BIDIRCPU']:
            self.properties.Set(pypbrtv3core.Property('path.maxdepth', [engine_settings.bidir_eyedepth]))
            self.properties.Set(pypbrtv3core.Property('light.maxdepth', [engine_settings.bidir_lightdepth]))
        elif engine in ['BIDIRVMCPU']:
            self.properties.Set(pypbrtv3core.Property('path.maxdepth', [engine_settings.bidirvm_eyedepth]))
            self.properties.Set(pypbrtv3core.Property('light.maxdepth', [engine_settings.bidirvm_lightdepth]))
            self.properties.Set(pypbrtv3core.Property('bidirvm.lightpath.count',
                                                 [engine_settings.bidirvm_lightpath_count]))
            self.properties.Set(pypbrtv3core.Property('bidirvm.startradius.scale',
                                                 [engine_settings.bidirvm_startradius_scale]))
            self.properties.Set(pypbrtv3core.Property('bidirvm.alpha', [engine_settings.bidirvm_alpha]))

        # Light strategy
        self.properties.Set(pypbrtv3core.Property('lightstrategy.type', engine_settings.lightstrategy_type))
    ...
    def __convert_realtime_settings(self):
        ...
        if engine_settings.device_preview == 'CPU':
            self.properties.Set(pypbrtv3core.Property('film.filter.type', 'BLACKMANHARRIS'))
            self.properties.Set(pypbrtv3core.Property('film.filter.width', 1.0))
        else:
            self.properties.Set(pypbrtv3core.Property('film.filter.type', 'NONE'))
        ...
        if engine_settings.use_opencl_always_enabled:
            ...
            self.properties.Set(pypbrtv3core.Property('opencl.code.alwaysenabled', enabled_features))
    ...
    def __convert_compute_settings(self):
        ...
        if not engine_settings.auto_threads:
            self.properties.Set(pypbrtv3core.Property('native.threads.count', engine_settings.native_threads_count))

        # OpenCL settings
        if engine_settings.opencl_settings_type == 'SIMPLE':
            self.properties.Set(pypbrtv3core.Property('opencl.cpu.use', engine_settings.opencl_use_all_cpus))
            self.properties.Set(pypbrtv3core.Property('opencl.gpu.use', engine_settings.opencl_use_all_gpus))
            self.properties.Set(pypbrtv3core.Property('opencl.devices.select', ''))

        elif engine_settings.opencl_settings_type == 'ADVANCED':
            self.properties.Set(pypbrtv3core.Property('opencl.cpu.use', True))
            self.properties.Set(pypbrtv3core.Property('opencl.gpu.use', True))

            if len(engine_settings.pbrtv3core_opencl_devices) > 0:
                dev_string = ''
                for dev_index in range(len(engine_settings.pbrtv3core_opencl_devices)):
                    dev = engine_settings.pbrtv3core_opencl_devices[dev_index]
                    dev_string += '1' if dev.opencl_device_enabled else '0'

                self.properties.Set(pypbrtv3core.Property('opencl.devices.select', dev_string))

        self.properties.Set(pypbrtv3core.Property('film.opencl.enable', engine_settings.film_use_opencl))

        kernelcache = engine_settings.kernelcache
        self.properties.Set(pypbrtv3core.Property('opencl.kernelcache', kernelcache))


    def __convert_epsilon(self):
        engine_settings = self.blender_scene.pbrtv3core_enginesettings
        self.properties.Set(pypbrtv3core.Property('scene.epsilon.min', engine_settings.epsilon_min))
        self.properties.Set(pypbrtv3core.Property('scene.epsilon.max', engine_settings.epsilon_max))
    ...
    def __convert_custom_props(self):
        ...
        if engine_settings.custom_properties:
            ...
            for prop in custom_params:
                prop = prop.split('=')
                self.properties.Set(pypbrtv3core.Property(prop[0], prop[1]))
    ...
    def __convert_all_channels(self):
        ...
        output_switcher_channel = pbrtv3_camera.pbrtv3core_imagepipeline.output_switcher_pass
        ...
        transparent_film = pbrtv3_camera.pbrtv3core_imagepipeline.transparent_film
    ...
    def __convert_lightgroups(self):
        if not self.blender_scene.pbrtv3_lightgroups.ignore:
            for i in range(len(self.pbrtv3core_exporter.lightgroup_cache)):
                self.convert_channel('RADIANCE_GROUP', i)

            for lg, id in self.pbrtv3core_exporter.lightgroup_cache.get_lightgroup_id_pairs():
