...
class DupliExporter(object):
    def __init__(self, pbrtv3core_exporter, blender_scene, duplicator, is_viewport_render=False):
    ...
    def convert(self, pbrtv3core_scene):
        self.properties = pypbrtv3core.Properties()
        export_settings = self.blender_scene.pbrtv3core_translatorsettings
        ...
        try:
            ...
            for particle_system in self.duplicator.particle_systems:
                if particle_system.settings.render_type in ['OBJECT', 'GROUP']:
                    ...
                    self.__convert_hair(pbrtv3core_scene, particle_system)
            ...
            if (has_particle_systems or is_duplicator_without_psys) and export_settings.export_particles:
                self.__convert_particles(pbrtv3core_scene)

        except Exception:
            log_exception(self.pbrtv3core_exporter, 'Could not convert particle systems of object %s' % self.duplicator.name)
        finally:
    ...
    def __report_progress(self, particle_system=None):
        ...
        self.pbrtv3core_exporter.renderengine.update_stats('Exporting...', message)


    def __convert_particles(self, pbrtv3core_scene):
        """
        ...
        for dupli_ob in obj.dupli_list:
            ...
            if dupli_ob.object not in self.pbrtv3core_exporter.instanced_duplis:
                self.pbrtv3core_exporter.instanced_duplis.add(dupli_ob.object)
        ...
        # Export all unique objects used by particle systems once, then only use their pbrtv3core names in the main loop below
        unique_objs = {}
        for do, dm, psys_name, persistent_id in duplis:
            if do.name not in unique_objs:
                ...
                object_exporter = ObjectExporter(self.pbrtv3core_exporter, self.blender_scene, self.is_viewport_render, do, 'dupli')
                object_exporter.convert(False, False, pbrtv3core_scene, None, dm)
        ...
        for do, dm, psys_name, persistent_id in duplis:
            ...
            if self.dupli_number % 10000 == 0:
                ...
                if self.pbrtv3core_exporter.renderengine.test_break():
                    return
            ...
            if do.type == 'LAMP':
                light_exporter = LightExporter(self.pbrtv3core_exporter, self.blender_scene, do, dupli_name_suffix)
                self.properties.Set(light_exporter.convert(pbrtv3core_scene, dm))
            else:
                ...
                for mat_index, exp_obj in enumerate(exported_objects):
                    ...
                    self.properties.Set(pypbrtv3core.Property(prefix + '.shape', exp_obj.pbrtv3core_shape_name))
                    self.properties.Set(pypbrtv3core.Property(prefix + '.material', exp_obj.pbrtv3core_material_name))
    ...
    def __convert_hair(self, pbrtv3core_scene, particle_system):
        """
        ...
        pbrtv3core_shape_name = ToValidPBRTv3CoreName(obj.name + '_' + psys.name)

        self.pbrtv3core_exporter.renderengine.update_stats('Exporting...', 'Refining Hair System %s' % psys.name)
        # Documentation: http://www.pbrtv3render.net/forum/viewtopic.php?f=8&t=12116&sid=03a16c5c345db3ee0f8126f28f1063c8#p112819
        pbrtv3core_scene.DefineStrands(pbrtv3core_shape_name, total_strand_count, len(points), points_as_tuples, segments,
        ...
        material_exporter = self.pbrtv3core_exporter.material_cache[material]
        ...
        prefix = 'scene.objects.' + pbrtv3core_shape_name
