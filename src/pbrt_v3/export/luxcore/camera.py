...
from ...outputs.pbrtv3core_api import pypbrtv3core, set_prop_cam
...
class CameraExporter(object):
    ...
    def __convert_viewport_camera(self):
        ...
        pbrtv3Camera = self.context.scene.camera.data.pbrtv3_camera if self.context.scene.camera is not None else None
        ...
        if pbrtv3Camera is not None:
            # arbitrary clipping plane
            self.__convert_clipping_plane(pbrtv3Camera)
            # Shutter open/close
            self.__convert_shutter(pbrtv3Camera)


    def __convert_final_camera(self):
        blCamera = self.blender_scene.camera
        blCameraData = blCamera.data
        pbrtv3Camera = blCameraData.pbrtv3_camera
        ...
        width, height = pbrtv3Camera.pbrtv3_film.resolution(self.blender_scene)
        ...
        if blCameraData.type == 'PERSP' and pbrtv3Camera.type == 'perspective':
        ...
        screenwindow = pbrtv3Camera.screenwindow(width, height, self.blender_scene, blCameraData,
                                              pbrtv3core_export=self.blender_scene.render.use_border)
        set_prop_cam(self.properties, 'screenwindow', screenwindow)

        if pbrtv3Camera.use_dof:
            # Do not world-scale this, it is already in meters
            lensradius = (blCameraData.lens / 1000.0) / (2.0 * pbrtv3Camera.fstop)
            set_prop_cam(self.properties, 'lensradius', lensradius)
            set_prop_cam(self.properties, 'autofocus.enable', pbrtv3Camera.autofocus)
    ...
    def __convert_camera_motion_blur(self, blCamera):
        pbrtv3Camera = blCamera.data.pbrtv3_camera

        # Note: enabling this in viewport leads to constant refresing of the render, even when cam is not animated
        if pbrtv3Camera.usemblur and pbrtv3Camera.cammblur and not self.is_viewport_render:
            ...
            anim_matrices = object_anim_matrices(self.blender_scene, blCamera, steps=pbrtv3Camera.motion_blur_samples)
        ...
        lookat = pbrtv3Camera.lookAt(blCamera)
    ...
    def __convert_clipping_plane(self, pbrtv3_camera_settings):
        if pbrtv3_camera_settings.enable_clipping_plane:
            obj_name = pbrtv3_camera_settings.clipping_plane_obj
    ...
    def __convert_shutter(self, pbrtv3_camera_settings):
        shutter_open, shutter_close = calc_shutter(self.blender_scene, pbrtv3_camera_settings)
