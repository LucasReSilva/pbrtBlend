...
class PBRTv3CoreExporter(object):
    def __init__(self, blender_scene, renderengine, is_viewport_render=False, context=None, is_material_preview=False):
        ...
        with pop_updated_scene_properties() and parse them into the pbrtv3core scene
        """
        ...
        self.config_properties = pypbrtv3core.Properties()
        self.scene_properties = pypbrtv3core.Properties()
        # All property changes since last pop_updated_scene_properties()
        self.updated_scene_properties = pypbrtv3core.Properties()
        ...
        # The summary is printed in core/__init__.py in pbrtv3core_render(), because otherwise it is drowned in a flood
    ...
    def pop_updated_scene_properties(self):
        ...
        """
        updated_properties = pypbrtv3core.Properties(self.updated_scene_properties)
        self.updated_scene_properties = pypbrtv3core.Properties()
    ...
    def convert(self, film_width, film_height, pbrtv3core_scene=None):
        """
        ...
        self.convert_camera()
        pbrtv3core_scene.Parse(self.pop_updated_scene_properties())
        ...
        if self.is_viewport_render and self.context.space_data.local_view:
            ...
            for blender_object in self.context.visible_objects:
                self.convert_object(blender_object, pbrtv3core_scene)

            background_props = pypbrtv3core.Properties()
            background_props.Set(pypbrtv3core.Property('scene.lights.LOCALVIEW_BACKGROUND.type', 'constantinfinite'))
        ...
        # Debug output
        if self.blender_scene.pbrtv3core_translatorsettings.print_cfg:
            print('\nConfig Properties:')
            print(self.config_properties)
        if self.blender_scene.pbrtv3core_translatorsettings.print_scn:
            print('\nScene Properties:')
        ...
        if self.blender_scene.pbrtv3core_translatorsettings.export_type == 'pbrtv3coreui':
        ...
        # Create pbrtv3core scene and config
        pbrtv3core_scene.Parse(self.pop_updated_scene_properties())
        pbrtv3core_config = pypbrtv3core.RenderConfig(self.config_properties, pbrtv3core_scene)
    ...
    def convert_imagepipeline(self):
        ...
        """
        temp_properties = pypbrtv3core.Properties()

        if self.blender_scene.camera is None:
            return temp_properties

        imagepipeline_settings = self.blender_scene.camera.data.pbrtv3_camera.pbrtv3core_imagepipeline
        ...
        if imagepipeline_settings.output_switcher_pass != 'disabled':
            ...
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'OUTPUT_SWITCHER'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.channel', channel))
            index += 1
        ...
        if tonemapper == 'TONEMAP_LINEAR' and imagepipeline_settings.use_auto_linear:
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'TONEMAP_AUTOLINEAR'))
            index += 1

        temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', tonemapper))
        ...
        if tonemapper == 'TONEMAP_LINEAR':
            ...
            # found for pbrtv3_camera.exposure_time(). This will nearly match viewport and final render, but while
            ...
            if not (self.is_viewport_render or export_to_pbrtv3coreui) and imagepipeline_settings.use_auto_linear:
                scale = imagepipeline_settings.linear_scale / 2.25
            else:
                scale = imagepipeline_settings.linear_scale

            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.scale', scale))
            ...
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.prescale', prescale))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.postscale', postscale))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.burn', burn))
        elif tonemapper == 'TONEMAP_LUXLINEAR':
            pbrtv3_camera = self.blender_scene.camera.data.pbrtv3_camera
            sensitivity = pbrtv3_camera.sensitivity
            #exposure = pbrtv3_camera.exposure_time() if not self.is_viewport_render else pbrtv3_camera.exposure_time() * 2.25
            fstop = pbrtv3_camera.fstop

            if self.is_viewport_render or export_to_pbrtv3coreui:
                exposure = pbrtv3_camera.exposure_time()
            else:
                exposure = pbrtv3_camera.exposure_time() / 2.25

            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.sensitivity', sensitivity))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.exposure', exposure))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.fstop', fstop))
        index += 1

        # Premultiply Alpha
        if imagepipeline_settings.transparent_film and not export_to_pbrtv3coreui and 'ALPHA' in self.passes_cache:
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'PREMULTIPLY_ALPHA'))
            index += 1
        ...
        if imagepipeline_settings.use_background_image:
            ...
            if os.path.exists(path) and os.path.isfile(path) and show_in_view:
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'BACKGROUND_IMG'))
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.file', path))
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.gamma', gamma))
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.storage', 'byte'))
                index += 1
        ...
        if imagepipeline_settings.use_mist:
            ...
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'MIST'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.color', color))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.amount', amount))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.startdistance', start))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.enddistance', end))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.excludebackground', exclude_background))
            index += 1
        ...
        if imagepipeline_settings.use_bloom:
            ...
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'BLOOM'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.radius', radius))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.weight', weight))
            index += 1

        # Color aberration
        if imagepipeline_settings.use_color_aberration:
            amount = imagepipeline_settings.color_aberration_amount
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'COLOR_ABERRATION'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.amount', amount))
            index += 1
        ...
        if imagepipeline_settings.use_vignetting:
            ...
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'VIGNETTING'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.scale', scale))
            index += 1
        ...
        if imagepipeline_settings.crf_type != 'NONE':
            ...
            if crf_name:
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'CAMERA_RESPONSE_FUNC'))
                temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.name', crf_name))
                index += 1

        # Contour lines for IRRADIANCE pass
        if imagepipeline_settings.output_switcher_pass == 'IRRADIANCE':
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'CONTOUR_LINES'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.range', imagepipeline_settings.contour_range))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.scale', imagepipeline_settings.contour_scale))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.steps', imagepipeline_settings.contour_steps))
            temp_properties.Set(
                pypbrtv3core.Property(prefix + str(index) + '.zerogridsize', imagepipeline_settings.contour_zeroGridSize))
            index += 1

        # Gamma correction: Blender expects gamma corrected image in realtime preview, but not in final render
        if self.is_viewport_render or export_to_pbrtv3coreui:
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.type', 'GAMMA_CORRECTION'))
            temp_properties.Set(pypbrtv3core.Property(prefix + str(index) + '.value', 2.2))
            index += 1
        ...
        if not self.blender_scene.pbrtv3_lightgroups.ignore:
            for lg, id in self.lightgroup_cache.get_lightgroup_id_pairs():
                if is_lightgroup_opencl_compatible(self, id):
                    ...
                    temp_properties.Set(pypbrtv3core.Property(prefix + str(id) + '.enabled', lg.lg_enabled))
                    temp_properties.Set(pypbrtv3core.Property(prefix + str(id) + '.globalscale', lg.gain))

                    if lg.use_rgb_gain:
                        temp_properties.Set(pypbrtv3core.Property(prefix + str(id) + '.rgbscale', list(lg.rgb_gain)))

                    if lg.use_temperature:
                        temp_properties.Set(pypbrtv3core.Property(prefix + str(id) + '.temperature', lg.temperature))
                elif verbose:
    ...
    def convert_object(self, blender_object, pbrtv3core_scene, update_mesh=True, update_material=True):
        ...
        new_properties = exporter.convert(update_mesh, update_material, pbrtv3core_scene)
        self.__set_scene_properties(new_properties)

        cache[obj_key] = exporter


    def convert_mesh(self, blender_object, pbrtv3core_scene, use_instancing, transformation):
        ...
        self.__convert_element(key, self.mesh_cache, exporter, pbrtv3core_scene)
    ...
    def convert_light(self, blender_object, pbrtv3core_scene):
        exporter = LightExporter(self, self.blender_scene, blender_object)
        self.__convert_element(get_elem_key(blender_object), self.light_cache, exporter, pbrtv3core_scene)
    ...
    def convert_duplis(self, pbrtv3core_scene, duplicator, dupli_system=None):
        exporter = DupliExporter(self, self.blender_scene, duplicator, self.is_viewport_render)
        self.__convert_element(get_elem_key(duplicator), self.dupli_cache, exporter, pbrtv3core_scene)
    ...
    def __convert_element(self, cache_key, cache, exporter, pbrtv3core_scene=None):
        ...
        new_properties = exporter.convert(pbrtv3core_scene) if pbrtv3core_scene else exporter.convert()
        self.__set_scene_properties(new_properties)
    ...
    def __convert_world_volume(self):
        ...
        properties = pypbrtv3core.Properties()
        ...
        if self.blender_scene.camera is not None:
            ...
            if cam_exterior_name in volumes:
                ...
                properties.Set(pypbrtv3core.Property('scene.world.volume.default', volume_exporter.pbrtv3core_name))
