...
class library_loader():
    ...
    platform_search = {
        'lzo': {
            'darwin': [
                bpy.utils.user_resource('SCRIPTS', 'addons/pbrtv3render/liblzo2.2.dylib'),
                bpy.app.binary_path[:-7] + ver_str + '/scripts/addons/pbrtv3render/liblzo2.2.dylib'
            ],
            'win32': [
                'lzo.dll',
                bpy.utils.user_resource('SCRIPTS', 'addons/pbrtv3render/lzo.dll'),
                bpy.app.binary_path[:-11] + ver_str + '/scripts/addons/pbrtv3render/lzo.dll'
            ],
            'linux': [
                '/usr/lib/liblzo2.so',
                '/usr/lib/liblzo2.so.2',
                bpy.app.binary_path[:-7] + ver_str + '/scripts/addons/pbrtv3render/liblzo2.so'
            ],
            'linux2': [
                '/usr/lib/liblzo2.so',
                '/usr/lib/liblzo2.so.2',
        ...
        'lzma': {
            'darwin': [
                bpy.utils.user_resource('SCRIPTS', 'addons/pbrtv3render/liblzmadec.dylib'),
                bpy.app.binary_path[:-7] + ver_str + '/scripts/addons/pbrtv3render/liblzmadec.dylib'
            ],
            'win32': [
                'lzma.dll',
                bpy.utils.user_resource('SCRIPTS', 'addons/pbrtv3render/lzma.dll'),
                bpy.app.binary_path[:-11] + ver_str + '/scripts/addons/pbrtv3render/lzma.dll'
            ],
            'linux': [
                '/usr/lib/liblzma.so',
                '/usr/lib/liblzma.so.2',
                bpy.app.binary_path[:-7] + ver_str + '/scripts/addons/pbrtv3render/liblzma.so'
            ],
            'linux2': [
                '/usr/lib/liblzma.so',
                '/usr/lib/liblzma.so.2',
...
class SmokeCache(object):
    ...
    this cache prevents that smoke is re-exported and pypbrtv3core.Properties are set just to check for volume updates.
    ...
    if PBRTv3Manager.CurrentScene.name == 'preview':
        ...
        if domain is not None:
            if bpy.app.version >= (2, 71, 0):
                ...
                smokecache = settings.point_cache
                ret = read_cache(smokecache, settings.use_high_resolution, settings.amplify + 1, flowtype)
                res_x = ret[0]
                res_y = ret[1]
                res_z = ret[2]
                density = ret[3]
                ...
                if settings.use_high_resolution:
                    big_res = [big_res[0] * (settings.amplify + 1), big_res[1] * (settings.amplify + 1),
