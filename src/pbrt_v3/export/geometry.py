...
class GeometryExporter(object):
    ...
    def __init__(self, pbrtv3_context, visibility_scene):
        self.pbrtv3_context = pbrtv3_context
                ...
                try:
                    ...
                    if not os.path.exists(ply_path) or not (self.visibility_scene.pbrtv3_engine.partial_ply and
                        ...
                        with open(ply_path, 'wb') as ply:
                            ...
                            ply.write(b'comment Created by LuxBlend 2.6 exporter for PBRTv3 - www.pbrtv3render.net\n')
        ...
        try:
            ...
            for i in iterator_range:
                try:
                    ...
                    if self.pbrtv3_context.API_TYPE == 'PURE':
    ...
    def exportShapeDefinition(self, obj, mesh_definition, parent=None):
        ...
        self.pbrtv3_context.objectBegin(me_name)

        # We need the transform in the object definition if this is a portal, since
        # an objectInstance won't be exported for it.
        if obj.type == 'MESH' and obj.data.pbrtv3_mesh.portal:
            self.pbrtv3_context.transform(matrix_to_list(obj.matrix_world, apply_worldscale=True))
        ...
        if ob_mat is not None:
            ...
            if object_is_emitter and \
                ...
                if not self.visibility_scene.pbrtv3_lightgroups.ignore:
                    self.pbrtv3_context.lightGroup(ob_mat.pbrtv3_emission.lightgroup, [])

                if not ob_mat.pbrtv3_material.nodetree:
                    self.pbrtv3_context.areaLightSource(*ob_mat.pbrtv3_emission.api_output(ob_mat))
                elif light_node is not None:
                    # Texture exporting
                    tex_maker = pbrtv3_texture_maker(self.pbrtv3_context, ob_mat.pbrtv3_material.nodetree)
                    self.pbrtv3_context.areaLightSource(*light_node.export(tex_maker.make_texture))

        self.pbrtv3_context.shape(me_shape_type, me_shape_params)
        self.pbrtv3_context.objectEnd()
    ...
    def exportShapeInstances(self, obj, mesh_definitions, matrix=None, parent=None):
        ...
        self.pbrtv3_context.attributeBegin(comment=obj.name, file=Files.GEOM)
        ...
        if is_object_animated:
            ...
            self.pbrtv3_context.motionBegin(step_times)

        # then export first matrix as normal
        if matrix is not None:
            self.pbrtv3_context.transform(matrix_to_list(matrix[0], apply_worldscale=True))
        else:
            self.pbrtv3_context.transform(matrix_to_list(obj.matrix_world, apply_worldscale=True))
        ...
        if is_object_animated:
            for next_matrix in next_matrices:
                ...
                # self.pbrtv3_context.motionInstance(me_name, i/fni, (i+1)/fni, '%s_motion_%i' % (obj.name, i))
                #self.pbrtv3_context.transformBegin(comment=obj.name)
                #self.pbrtv3_context.identity()
                #self.pbrtv3_context.transform(matrix_to_list(next_matrix, apply_worldscale=True))
                #self.pbrtv3_context.coordinateSystem('%s_motion_%i' % (obj.name, i))
                #self.pbrtv3_context.transformEnd()
                self.pbrtv3_context.transform(matrix_to_list(next_matrix, apply_worldscale=True))
            self.pbrtv3_context.motionEnd()
        ...
        for me_name, me_mat_index, me_shape_type, me_shape_params in mesh_definitions:
            ...
            if use_inner_scope:
                self.pbrtv3_context.attributeBegin()
            ...
            if ob_mat is not None:
                ...
                if self.pbrtv3_context.API_TYPE == 'FILE':
                    self.pbrtv3_context.set_output_file(Files.MATS)
                    mat_export_result = ob_mat.pbrtv3_material.export(self.visibility_scene, self.pbrtv3_context,
                                                                         ob_mat, mode='indirect')
                    self.pbrtv3_context.set_output_file(Files.GEOM)

                    if not 'CLAY' in mat_export_result:
                        self.pbrtv3_context.namedMaterial(ob_mat.name)
                elif self.pbrtv3_context.API_TYPE == 'PURE':
                    mat_export_result = ob_mat.pbrtv3_material.export(self.visibility_scene, self.pbrtv3_context,
                                                                         ob_mat, mode='direct')
                ...
                if int_v:
                    self.pbrtv3_context.interior(int_v)
                elif self.geometry_scene.pbrtv3_world.default_interior_volume:
                    self.pbrtv3_context.interior(self.geometry_scene.pbrtv3_world.default_interior_volume)
                if ext_v:
                    self.pbrtv3_context.exterior(ext_v)
                elif self.geometry_scene.pbrtv3_world.default_exterior_volume:
                    self.pbrtv3_context.exterior(self.geometry_scene.pbrtv3_world.default_exterior_volume)
            ...
            # ordinary mesh instance
                ...
                self.pbrtv3_context.objectInstance(me_name)

            if use_inner_scope:
                self.pbrtv3_context.attributeEnd()
    ...
    def handler_Duplis_PATH(self, obj, *args, **kwargs):
        ...
        if psys.settings.pbrtv3_hair.use_binary_output:
            ...
            info = 'Created by LuxBlend 2.6 exporter for PBRTv3 - www.pbrtv3render.net'
            ...
            self.pbrtv3_context.attributeBegin('hairfile_%s' % partsys_name)
            self.pbrtv3_context.transform(matrix_to_list(obj.matrix_world, apply_worldscale=True))
            self.pbrtv3_context.namedMaterial(hair_mat.name)
            ...
            self.pbrtv3_context.shape('hairfile', hair_shape_params)
            self.pbrtv3_context.attributeEnd()
            self.pbrtv3_context.set_output_file(Files.MATS)
            mat_export_result = hair_mat.pbrtv3_material.export(self.visibility_scene, self.pbrtv3_context, hair_mat,
                                                                   mode='indirect')
            self.pbrtv3_context.set_output_file(Files.GEOM)
            ...
            for sn, si, st, sp in hair_Junction:
                self.pbrtv3_context.objectBegin(sn)
                self.pbrtv3_context.shape(st, sp)
                self.pbrtv3_context.objectEnd()
...
@persistent
def pbrtv3_scene_update(context):
...
if hasattr(bpy.app, 'handlers') and hasattr(bpy.app.handlers, 'scene_update_post'):
    bpy.app.handlers.scene_update_post.append(pbrtv3_scene_update)
    bpy.app.handlers.load_post.append(pbrtv3_scene_load)
