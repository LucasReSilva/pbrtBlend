...
def attr_light(scene, pbrtv3_context, light, name, group, light_type, params, transform=None, portals=[]):
    """
    pbrtv3_context		pypbrtv3.Context
    name			string
    ...
    light_type to context pbrtv3_context. The lightSource will be
    ...
    if transform is not None:
        pbrtv3_context.transformBegin(comment=name, file=Files.MAIN)
        pbrtv3_context.transform(transform)
    else:
        pbrtv3_context.attributeBegin(comment=name, file=Files.MAIN)

    if light.type == 'SPOT' and light.pbrtv3_lamp.pbrtv3_lamp_spot.projector:
        pbrtv3_context.rotate(180, 0, 1, 0)

    pbrtv3_context.lightGroup(group, [])

    mirrorTransform = light.type == 'HEMI'

    if mirrorTransform:
        # correct worldmap orientation
        pbrtv3_context.transformBegin(file=Files.MAIN)
        pbrtv3_context.scale(-1, 1, 1)

    if light.pbrtv3_lamp.Exterior_volume:
        pbrtv3_context.exterior(light.pbrtv3_lamp.Exterior_volume)
    elif scene.pbrtv3_world.default_exterior_volume:
        pbrtv3_context.exterior(scene.pbrtv3_world.default_exterior_volume)

    pbrtv3_context.lightSource(light_type, params)

    if mirrorTransform:
        pbrtv3_context.transformEnd()

    for portal in portals:
        pbrtv3_context.portalInstance(portal)
...
def exportLight(scene, pbrtv3_context, ob, matrix, portals=[]):
    light = ob.data
    ...
    if light.type == 'SUN':
        ...
        if not legacy_sky and sunsky_type not in ['sun', 'distant']:  # new skymodel
            if sunsky_type == 'sky':
                attr_light(scene, pbrtv3_context, light, ob.name, light_group, 'sky2', light_params, portals=portals)
            elif sunsky_type == 'sunsky':
                attr_light(scene, pbrtv3_context, light, ob.name, light_group, 'sunsky2', light_params, portals=portals)
        else:  # Use light definition itself, old sky model, sun only, or distant, as needed.
            attr_light(scene, pbrtv3_context, light, ob.name, light_group, sunsky_type, light_params, portals=portals)

        return True

    if light.type == 'HEMI':
        infinite_type = 'infinitesample' if light.pbrtv3_lamp.pbrtv3_lamp_hemi.hdri_infinitesample else 'infinite'
        attr_light(scene, pbrtv3_context, light, ob.name, light_group, infinite_type, light_params,
    ...
    if light.type == 'SPOT':
        ...
        attr_light(scene, pbrtv3_context, light, ob.name, light_group, light_type, light_params,
                   transform=matrix_to_list(matrix, apply_worldscale=True), portals=portals)
        return True
    ...
    if light.type == 'POINT':
        ...
        if light.pbrtv3_lamp.pbrtv3_lamp_point.usesphere and \
            ...
            pbrtv3_context.attributeBegin(ob.name, file=Files.MAIN)
            pbrtv3_context.transform(matrix_to_list(matrix, apply_worldscale=True))
            pbrtv3_context.lightGroup(light_group, [])
            ...
            if light.pbrtv3_lamp.pbrtv3_lamp_point.null_lamp:
                ...
                pbrtv3_context.makeNamedMaterial(ob.name, mat_params)

                pbrtv3_context.namedMaterial(ob.name)
            ...
            pbrtv3_context.scale(1 / x, 1 / y, 1 / z)
            ...
            pbrtv3_context.shape('sphere', shape_params)
            ...
            attr_light(scene, pbrtv3_context, light, ob.name, light_group, 'point', light_params,
                       transform=matrix_to_list(matrix, apply_worldscale=True), portals=portals)
        return True
    ...
    if light.type == 'AREA':
        ...
        if light.pbrtv3_lamp.pbrtv3_lamp_area.null_lamp:
            ...
            if scene.pbrtv3_rendermode.renderer == 'pbrtv3core':
...
def lights(pbrtv3_context, geometry_scene, visibility_scene, mesh_definitions):
    """
    pbrtv3_context		pypbrtv3.Context
    Iterate over the given scene's light sources,
    and export the compatible ones to the context pbrtv3_context.
    ...
    for ob in geometry_scene.objects:
        ...
        if ob.is_duplicator and ob.dupli_type in ('GROUP', 'VERTS', 'FACES'):
            ...
            for dupli_ob in ob.dupli_list:
                ...
                have_light |= exportLight(visibility_scene, pbrtv3_context, dupli_ob.object, dupli_ob.matrix,
                                          portal_shapes)
            ...
            if ob.type == 'LAMP':
                have_light |= exportLight(visibility_scene, pbrtv3_context, ob, ob.matrix_world, portal_shapes)
