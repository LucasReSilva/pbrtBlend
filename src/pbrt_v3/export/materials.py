...
class ExportedTextures(object):
    ...
    @staticmethod
    def texture(pbrtv3_context, name, type, texture, params):
        if pbrtv3_context.API_TYPE == 'PURE':
            pbrtv3_context.texture(name, type, texture, params)
    ...
    def export_new(pbrtv3_context):
        for n, ty, tx, p in zip(
            ...
            if pbrtv3_context.API_TYPE != 'PURE' and n not in ExportedTextures.exported_texture_names:
                pbrtv3_context.texture(n, ty, tx, p)
...
class ExportedMaterials(object):
    ...
    @staticmethod
    def makeNamedMaterial(pbrtv3_context, name, paramset):
        if pbrtv3_context.API_TYPE == 'PURE':
            pbrtv3_context.makeNamedMaterial(name, paramset)
            return
    ...
    @staticmethod
    def export_new_named(pbrtv3_context):
        for n, p in zip(ExportedMaterials.material_names, ExportedMaterials.material_psets):
            if pbrtv3_context.API_TYPE != 'PURE' and n not in ExportedMaterials.exported_material_names:
                pbrtv3_context.makeNamedMaterial(n, p)
...
def convert_texture(scene, texture, variant_hint=None):
    ...
    pbrtv3_tex_name = 'blender_%s' % texture.type.lower()
    ...
    if texture.type == 'DISTORTED_NOISE':
        pbrtv3_tex_name = 'blender_distortednoise'
    ...
    # Translate Blender Image/movie into pbrtv3 tex
    if texture.type == 'IMAGE' and texture.image and texture.image.source in ['GENERATED', 'FILE', 'SEQUENCE']:
        ...
        if texture.image.source == 'GENERATED':
            tex_image = 'pbrtv3blend_baked_image_%s.%s' % (
    ...
    return variant, pbrtv3_tex_name, paramset
...
def add_texture_parameter(pbrtv3_context, pbrtv3_prop_name, variant, property_group, value_transform_function=None):
    """
    pbrtv3_context				pypbrtv3.Context - like object
    pbrtv3_prop_name			PBRTv3 material/texture parameter name
    ...
    if hasattr(property_group, '%s_use%stexture' % (pbrtv3_prop_name, variant)):
        export_param_name = getattr(property_group, pbrtv3_prop_name)

        if value_transform_function is None:
            value_transform_function = value_transform_passthrough

        if getattr(property_group, '%s_use%stexture' % (pbrtv3_prop_name, variant)):
            texture_name = getattr(property_group, '%s_%stexturename' % (pbrtv3_prop_name, variant))
            if texture_name:
                with TextureCounter(texture_name):
                    ...
                    if texture != False:
                        if texture.pbrtv3_texture.type != 'BLENDER':
                            ...
                            pbrtv3_tex_variant, paramset = tex_pbrtv3_texture.get_paramset(PBRTv3Manager.CurrentScene,
                                                                                           texture)
                            if pbrtv3_tex_variant == variant:
                                ExportedTextures.texture(pbrtv3_context, texture_name, variant, tex_pbrtv3_texture.type,
                                                         paramset)
                            else:
                                PBRTv3Log('WARNING: Texture %s is wrong variant; needed %s, got %s' % (
                                    pbrtv3_prop_name, variant, pbrtv3_tex_variant))
                        else:
                            ...
                            if pbrtv3_tex_variant == variant:
                                ExportedTextures.texture(pbrtv3_context, texture_name, pbrtv3_tex_variant, pbrtv3_tex_name,
                                                         paramset)
                            else:
                                PBRTv3Log('WARNING: Texture %s is wrong variant; needed %s, got %s' % (
                                    pbrtv3_prop_name, variant, pbrtv3_tex_variant))

                        if hasattr(property_group, '%s_multiplyfloat' % pbrtv3_prop_name) and \
                                getattr(property_group, '%s_multiplyfloat' % pbrtv3_prop_name):
                            ...
                            ExportedTextures.texture(
                                ...
                                'scale',
                                ParamSet() \
                                .add_float('tex1', float(getattr(property_group, '%s_floatvalue' % pbrtv3_prop_name))) \
                                .add_texture('tex2', texture_name)
                            )
                            texture_name += '_scaled_%i' % sv

                        if hasattr(property_group, '%s_multiplyfresnel' % pbrtv3_prop_name) and \
                                getattr(property_group, '%s_multiplyfresnel' % pbrtv3_prop_name):
                            sv = ExportedTextures.next_scale_value()
                            ExportedTextures.texture(
                                pbrtv3_context,
                                '%s_scaled_%i' % (texture_name, sv),
                                ...
                                ParamSet() \
                                .add_float('tex1',
                                           float(getattr(property_group, '%s_fresnelvalue' % pbrtv3_prop_name))) \
                                .add_texture('tex2', texture_name)
                            )
                            texture_name += '_scaled_%i' % sv

                        if hasattr(property_group, '%s_multiplycolor' % pbrtv3_prop_name) and \
                                getattr(property_group, '%s_multiplycolor' % pbrtv3_prop_name):
                            sv = ExportedTextures.next_scale_value()
                            ExportedTextures.texture(
                                pbrtv3_context,
                                '%s_scaled_%i' % (texture_name, sv),
                                ...
                                .add_color(
                                    ...
                                    [float(value_transform_function(i)) for i in
                                     getattr(property_group, '%s_color' % pbrtv3_prop_name)]
                                ) \
                                .add_texture('tex2', texture_name)
                            )
                            texture_name += '_scaled_%i' % sv
            ...
            if variant == 'float':
                fval = float(getattr(property_group, '%s_floatvalue' % pbrtv3_prop_name))
                if not getattr(property_group, '%s_ignore_unassigned' % pbrtv3_prop_name):
                    params.add_float(
                        export_param_name,
                        fval
                    )
                ...
                params.add_color(
                    export_param_name,
                    [float(value_transform_function(i)) for i in getattr(property_group, '%s_color' % pbrtv3_prop_name)]
                )
            elif variant == 'fresnel':
                fval = float(getattr(property_group, '%s_fresnelvalue' % pbrtv3_prop_name))
                params.add_float(
                    export_param_name,
                    fval
                )
    else:
        PBRTv3Log('WARNING: Texture %s is unsupported variant; needed %s' % (pbrtv3_prop_name, variant))
