...
class PBRTv3_OT_volume_add(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.volume_add"
...
class PBRTv3_OT_volume_remove(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.volume_remove"
...
class PBRTv3_OT_lightgroup_add(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.lightgroup_add"
...
class PBRTv3_OT_lightgroup_remove(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.lightgroup_remove"
...
class PBRTv3_OT_materialgroup_add(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.materialgroup_add"
...
class PBRTv3_OT_materialgroup_remove(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.materialgroup_remove"
...
class PBRTv3_OT_opencl_device_list_update(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.opencl_device_list_update"
    bl_label = "Update the OpenCL device list"

    def invoke(self, context, event):
        devs = context.scene.pbrtv3core_enginesettings.pbrtv3core_opencl_devices
        # Clear the list
        ...
        deviceList = pypbrtv3core.GetOpenCLDeviceList()
...
@PBRTv3Addon.addon_register_class
class PBRTv3_OT_update_pbrtv3blend(bpy.types.Operator):
    """Update LuxBlend to the latest version"""
    bl_idname = "pbrtv3render.update_pbrtv3blend"
    ...
    def execute(self, context):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_path:
            ...
            url = 'https://bitbucket.org/pbrtv3render/pbrtv3blend25/get/default.zip'
            try:
            ...
            with zipfile.ZipFile(temp_zip_path) as zip:
                for member in zip.namelist():
                    if 'src/pbrtv3render' in member:
                        # Remove the first two directories and the filename
                        # e.g. pbrtv3render-pbrtv3blend25-bfb488c84111/src/pbrtv3render/ui/textures/wrinkled.py
                        # becomes pbrtv3render/ui/textures/
            ...
            extracted_pbrtv3blend_path = os.path.join(temp_dir_path, 'pbrtv3render')

            if not os.path.exists(extracted_pbrtv3blend_path):
                self.report({'ERROR'}, 'Could not extract ZIP archive! Aborting.')
                return {'FINISHED'}

            # Find the old LuxBlend files
            pbrtv3blend_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
            print('LuxBlend addon folder:', pbrtv3blend_dir)
            ...
            # remove __init__.py
            os.remove(os.path.join(pbrtv3blend_dir, '__init__.py'))
            # remove all folders
            DIRNAMES = 1
            for dir in next(os.walk(pbrtv3blend_dir))[DIRNAMES]:
                shutil.rmtree(os.path.join(pbrtv3blend_dir, dir))

            print('Copying new LuxBlend files')
            # copy new LuxBlend files
            # copy __init__.py
            shutil.copy2(os.path.join(extracted_pbrtv3blend_path, '__init__.py'), pbrtv3blend_dir)
            # copy all folders
            recursive_overwrite(extracted_pbrtv3blend_path, pbrtv3blend_dir)
...
class PBRTv3_OT_open_daily_builds_webpage(bpy.types.Operator):
    ...
    bl_idname = 'pbrtv3render.open_daily_builds_webpage'
    bl_label = 'Update PBRTv3'

    def execute(self, context):
        import webbrowser
        webbrowser.open('http://www.pbrtv3render.net/forum/viewtopic.php?f=30&t=12147')
        return {'FINISHED'}
...
class PBRTv3_OT_fix_color_management(bpy.types.Operator):
    ...
    bl_idname = "pbrtv3render.fix_color_management"
